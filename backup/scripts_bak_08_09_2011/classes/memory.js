...
.module("memory", function ( $ ) { "use strict";
	...
	function poly_buf_readSegment( offset, len ) {
		...
		return readValue(this.buf
			, (this.addrA20 + offset) - this.addrStart_buf
			, len);
	}
	...
	if ( jemul8.support.typedDataView ) {
		...
		readValue = function ( buf, addrA20, len ) {
			try {
				switch ( len ) {
				case 1:	// Byte (8-bit)
				...
				default:
					jemul8.problem("readValue() :: Operand-size > 32-bit");
				}
			} catch ( e ) {
				return 0;
			}
		...
		//	(A20 should already be applied)
		writeValue = function ( buf, addrA20, val, len ) {
			...
			case 4:	// Dword (32-bit / 4-bytes)
				...
				return;
			default:
				jemul8.problem("writeValue() :: Operand-size > 32-bit");
			}
		};
	...
	//	is not available)
		...
		writeValue = function ( buf, addrA20, val, len ) {
			...
			//	, "writeValue() :: Value is greater in bytes than size");
			/* ==== /Guards ==== */
