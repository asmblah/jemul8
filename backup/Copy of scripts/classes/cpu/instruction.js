...
var mod = new jsEmu.SecondaryModule( function ( jsEmu, machine, motherboard, CPU, FlashBIOSChip, BIOS, DRAM ) {
	...
	function Instruction( offsetAddress, name, operand1, operand2, lenBytes, sizeAddress_Bytes, sizeOperand_Bytes ) {
		/* ==== Guards ==== */
	...
	var hsh_func_insnExecute = {
		...
		// Procedure Call
			...
			var EIP = CPU.EIP.Get();
			/* ==== /Malloc ==== */
			switch ( this.typeJump ) {
			// Near call, within current code segment / intrasegment call
			case "NEAR_REL":
			...
			case "NEAR_ABS":
				...
				break;
			// Far call, other code segment / intersegment call
			case "FAR_ABS":
				// General Protection fault / exception if InstructionPointer goes out of bounds for the current Code Segment
				if ( !this.InCodeSegmentLimits(EIP) ) { CPUException("GP", 0); return; }
		...
		// Unsigned Divide
			...
			if ( /*dividend == 0 || */divisor == 0 ) { Interrupt(0); return; }
		...
		// Signed Integer Division
			...
			/* ==== /Malloc ==== */
			// Divide by Zero - CPU Interrupt
			if ( divisor == 0 ) { Interrupt(0); return; }
			// Integer result - truncated toward zero
			res = (dividend / divisor) >> 0;
			// Dividend is AX
			if ( sizeOperand_Bytes == 2 ) {
		...
		// Signed Multiply
			...
			SetFlags(multiplicand, multiplier, res);
		...
		//	Based on http://pdos.csail.mit.edu/6.828/2005/readings/i386/IRET.htm
			...
			// Protected Mode
			if ( CPU.PE.Get() ) {
				// Virtual-8086 Mode ( not allowed )
				if ( CPU.VM.Get() ) { CPUException("GP", 0); return; }
				
				if ( CPU.NT.Get() ) {
					throw new Error( "IRET: unsupported stage TASK-RETURN" );
				} else {
					throw new Error( "IRET: unsupported stage STACK-RETURN" );
				}
			// Real-address Mode
			} else {
				// IRET
				if ( sizeOperand_Bytes == 16 ) {
					CPU.IP.Set(CPU.PopStack());
				// IRETD
				} else {
					CPU.IP.Set(CPU.PopStack(4));
				}
		...
		// Pop word off stack top ( SS:SP )
			...
			case "SEGMENT":
				...
				if ( this.operand1.isPointer ) {
					DRAM.WriteBytesRAM(this.operand1.GetPointerAddress(), CPU.PopStack(), 2);
					return;
				} else {
					this.operand1.reg.Set(CPU.PopStack());
					return;
				}
			// Immediate data
			case "IMMEDIATE":
		...
		// Push data onto stack top ( SS:SP )
			...
			case "SEGMENT":
				// Memory pointer; read
				if ( this.operand1.isPointer ) {
					CPU.PushStack(DRAM.ReadBytesRAM(this.operand1.GetPointerAddress(), sizeOperand_Bytes), sizeOperand_Bytes);
					return;
				} else {
					CPU.PushStack(this.operand1.reg.Get(), sizeOperand_Bytes);
					return;
				}
			// Immediate data
			case "IMMEDIATE":
				...
				if ( this.operand1.isPointer ) {
					...
					CPU.PushStack(this.operand1.immed, sizeOperand_Bytes);
					return;
				}
			// Unsupported data... ?!
			default:
	...
	function Interrupt( num ) {
		...
		if ( num in CPU.hsh_InterruptHandler ) {
			CPU.hsh_InterruptHandler[num]();
			return;
