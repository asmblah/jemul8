...
var mod = new jsEmu.PrimaryModule( function ( jsEmu ) {
	...
	Register.prototype.Set = function ( val ) {
		/* ==== Guards ==== */
		//jsEmu.Assert(val >= 0, "Register.Set :: tried to set a register to a negative value (hint: use two's complement).");
		/* ==== /Guards ==== */
		
		// Wrap round if too large
		//if ( val > this.bitmaskSize ) val -= this.valNegativeWrap;
	...
	function SubRegister( name, sizeBytes, regMaster, bitmaskSize, bytesInLeft ) {
		...
		jsEmu.Assert(regMaster && regMaster instanceof jsEmu.Register, "SubRegister constructor :: no valid master register specified.");
		/* ==== /Guards ==== */
