...
.module("dram", function ( $ ) { "use strict";
	...
	x86DRAM.prototype.readBytes = function ( addr, num ) {
		...
		default:
			jemul8.problem("x86DRAM.readBytes :: Operand size > 32-bit not supported");
		}
	};
	...
	x86DRAM.prototype.writeBytes = function ( addr, val, num ) {
		...
		jemul8.assert(val === (val & jemul8.generateMask(num))
			, "x86DRAM.writeBytes :: Value is greater in bytes than size");
		/* ==== /Guards ==== */
		...
		case 4:	// Dword ( 32-bit )
			...
			return;
		default:
			jemul8.problem("x86DRAM.writeBytes :: Operand size > 32-bit not supported");
		}
	};
