...
.module("iodev/dma", function ( $ ) { "use strict";
	...
	DMA.prototype.setDRQ = function ( idx_channel, val ) {
		...
		// ( NB: isSlave will be converted to int )
		if ( (baseDMA & (0x7FFF0000 << isSlave))
				!== (roofDMA & (0x7FFF0000 << isSlave)) ) {
			...
			jemul8.panic("DMA.setDRQ() :: Request outside "
				+ $.format("hex", 64 << isSlave) + "k boundary");
			return 0;
		}
	...
	DMA.prototype.unregisterDMAChannel = function ( idx_channel ) {
		...
		(idx_channel > 3 ? this.DMA2 : this.DMA1)
			.list_channel[ idx_channel & 0x03 ].inUse = 0;
	...
	DMA_Core.prototype.controlHRQ = function () {
		var machine = this.cntrlr.machine
			, idx_channel;
	...
	function readHandler( device, addr, io_len ) {
		...
		case 0xDA: // DMA-2 Temporary register
			jemul8.problem("DMA-" + (isSlave + 1) + " readHandler() :: Read of temporary register always returns 0");
			return 0;
	...
	function writeHandler( device, addr, val, io_len ) {
		var state = device.state // "device" will be DMA
			, core;
		
		jemul8.info("DMA writeHandler() :: Write to address: " + $.format("hex", addr) + " = " + $.format("hex", val));
		
		/** NB: This is an 8237 DMA **/
