...
.module("io", function ( $ ) { "use strict";
	...
	= function ( device, addr, name_portPart, fn, mask ) {
		/* ==== Guards ==== */
		$.assert(device && (device instanceof x86Emu.IODevice)
		...
		// IO port conflict
			...
			$.problem("IO.registerIO_Read() :: IO port conflict - "
	...
	= function ( device, addr, name_portPart, fn, mask ) {
		...
		// IO port conflict
			...
			$.problem("IO.registerIO_Write() :: I/O port conflict - "
				+ $.format("hex", addr) + " already assigned as "
				+ port.name_port);
			return false;
		}
	...
	IO.prototype.registerIRQ = function ( device, irq, namePart ) {
		...
		$.assert(!isNaN(irq) && irq === parseInt(irq)
		...
		if ( irq < 0 ) {
			return $.panic("IO.registerIRQ() :: IO device '"
		...
		if ( (nameExisting = this.hsh_irq_nameHandler[ irq ]) !== null ) {
			return $.panic("IO.registerIRQ() :: IRQ #" + irq
	...
	IO.prototype.unregisterIRQ = function ( device, irq, namePart ) {
		...
		// IRQ index out of bounds
		if ( irq < 0 ) {
			return $.panic("IO.unregisterIRQ() :: IO device '"
