...
.module("iodev/vga", function ( $ ) { "use strict";
	...
	function VGA( machine ) {
		$.assert(this && (this instanceof VGA), "VGA ctor ::"
	...
	$.inherit(VGA, x86Emu.IODevice, "VGA"); // Inheritance
	VGA.prototype.init = function () {
		var idx, state = this.state
			, x, y;
		...
		this.bufVRAM = x86Emu.allocBuffer(state.size_mem);
	...
	VGA.prototype.update = function () {
		...
		if ( state.graphics_ctrl.graphics_alpha ) {
			...
			if ( (iWidth != old_iWidth) || (iHeight != old_iHeight)
					|| (state.last_bpp > 8) ) {
				if ( trace ) { $.warning("dimension_update()"); }
					...
					for ( yc = 0, yti = 0 ; yc < iHeight ; yc += Y_TILESIZE, yti++ ) {
						for ( xc = 0, xti = 0 ; xc < iWidth ; xc += X_TILESIZE, xti++ ) {
							if ( this.getTileUpdated(xti, yti) ) {
								...
								if ( trace ) { $.warning("graphics_tile_update()"); }
			...
			case 3: // [Bochs] FIXME: is this really the same ???
				if ( state.sequencer.chain_four ) {
					...
					if ( state.misc_output.select_high_bank != 1 ) {
						$.panic("VGA.update() :: select_high_bank != 1");
					}
		...
		// Text mode
			...
			if ( (state.vga_mem_updated == 0) && (this.update_static.cs_counter > 0) ) {
				return;
			}
			...
			if ( (rows * tm_info.line_offset) > (1 << 17) ) {
				$.problem("VGA.update() :: Text mode: out of memory");
				return;
			}
			...
			if ( state.vga_mem_updated && this.bufVRAM[ start_address ] ) { debugger; }
	...
	function readHandler( device, addr, io_len ) {
		...
		if ( trace ) {
			$.info("VGA readHandler() :: Read from address: "
				+ $.format("hex", addr));
		}
		...
		function debug( ret ) {
			if ( trace ) {
				if ( io_len === 1 ) {
					$.debug("VGA readHandler() :: 8-bit read from "
						+ $.format("hex", addr) + " = "
						+ $.format("hex", ret));
				} else {
					$.debug("VGA readHandler() :: 16-bit read from "
						+ $.format("hex", addr) + " = "
						+ $.format("hex", ret));
				}
			}
		...
		//	$.panic("VGA readHandler() :: BX_VIDEO_DIRECT - unsupported");
		...
		case 0x03C0: // ???
			if ( state.attribute_ctrl.flip_flop == 0 ) {
				...
				$.problem("VGA readHandler() :: I/O read: 0x3C0: flip_flop != 0");
				return 0;
			}
			break;
		...
		case 0x03C1: // ???
			...
			default:
				if ( trace ) {
					$.info("VGA readHandler() :: I/O read: 0x3C1: unknown register "
		...
		case 0x03c2: // Input Status 0
			if ( trace ) {
				$.debug("VGA readHandler() :: I/O read 0x3C2:"
		...
		case 0x03C5: // Sequencer Registers 00 -> 04
			...
			case 0: // Sequencer: reset
				if ( trace ) {
					$.debug("VGA readHandler() :: I/O read 0x3C5:"
						+ " sequencer reset");
				}
			...
			default:
				if ( trace ) {
					$.debug("VGA readHandler() I/O read 0x3C5: index "
		...
		case 0x03CD: // ???
			if ( trace ) {
				$.debug("VGA readHandler() I/O read from 03cd");
			}
			return debug(0x00);
		...
		case 0x03CF: // Graphics Controller Registers 00 -> 08
			...
			case 5: // Mode
				...
				if ( state.graphics_ctrl.odd_even
						|| state.graphics_ctrl.shift_reg ) {
					if ( trace ) {
						$.debug("VGA readHandler() :: I/O read 0x3CF: reg 05 = "
							+ $.format("hex", retval));
					}
				}
				return debug(retval);
			...
			default:
				// ???
				if ( trace ) {
					$.debug("VGA readHandler() I/O read: 0x3CF: index "
		...
		case 0x03D5: // CRTC Registers (color emulation modes)
			if ( state.CRTC.address > 0x18 ) {
				if ( trace ) {
					$.debug("VGA readHandler() :: I/O read: invalid CRTC register "
		...
		default:
			...
			if ( trace ) {
				$.info("VGA readHandler() :: Unsupported read, address="
					+ $.format("hex", addr) + "!");
			}
			return debug(0);
		}
	...
	function writeHandler( device, addr, val, io_len, /* private */ noLog ) {
		...
		if ( trace ) {
			$.info("VGA writeHandler() :: Write to address: "
		...
		if ( trace && !noLog ) {
			if ( io_len === 1 ) {
				$.debug(" :: 8-bit write to "
					+ $.format("hex", addr) + " = " + $.format("hex", val));
			} else if ( io_len === 2 ) {
				$.debug(" :: 16-bit write to "
					+ $.format("hex", addr) + " = " + $.format("hex", val));
			} else {
				$.panic(" :: Strange VGA write size "
					+ io_len);
			}
		}
		...
		// For OS/2
		//if ( bx_options.videomode === BX_VIDEO_DIRECT ) {
		//	$.panic(" :: BX_VIDEO_DIRECT - unsupported");
		...
		case 0x03BA: // Feature Control (monochrome emulation modes)
			if ( trace ) {
				$.debug(" :: I/O write 3BA: feature control: ignoring");
			}
			break;
		...
		case 0x03C0: // Attribute Controller
			if ( state.attribute_ctrl.flip_flop == 0 ) { // Address mode
				...
				if ( trace ) {
					$.debug(" :: I/O write 3C0: video_enabled = "
						+ state.attribute_ctrl.video_enabled);
				}
				
				if ( !state.attribute_ctrl.video_enabled ) {
					//bx_gui->clear_screen();
					if ( trace ) { $.warning("clear_screen()"); }
				} else if ( !prev_video_enabled ) {
					if ( trace ) { $.debug(" :: Found enable-transition"); }
				...
				default:
					if ( trace ) {
						$.debug(" :: I/O write 0x3C0: address mode reg="
							+ $.format("hex", val));
					}
				}
				...
				case 0x11: // Overscan Color Register
					...
					if ( trace ) {
						$.debug(" :: I/O write 0x3C0: overscan color = "
							+ $.format("hex", val));
					}
					break;
				case 0x12: // Color Plane Enable Register
					...
					if ( trace ) {
						$.debug(" :: I/O write 0x3c0: color plane enable = "
							+ $.format("hex", val));
					}
					break;
				case 0x13: // Horizontal Pixel Panning Register
					...
					if ( trace ) {
						$.debug(" :: I/O write 0x3C0: horiz pel panning = "
							+ $.format("hex", val));
					}
					break;
				case 0x14: // Color Select Register
					...
					if ( trace ) {
						$.debug(" :: I/O write 0x3C0: color select = "
							+ $.format("hex", state.attribute_ctrl.color_select));
					}
					break;
				default:
					if ( trace ) {
						$.debug(" :: I/O write 0x3C0: data-write mode "
							+ $.format("hex", state.attribute_ctrl.address));
					}
				}
			}
		...
		case 0x03C3: // VGA enable
			...
			if ( trace ) {
				$.debug(" :: I/O write 0x03C3: VGA enable ="
					+ state.vga_enabled);
			}
			break;
		...
		case 0x03C4: // Sequencer Index Register
			if ( val > 4 ) {
				if ( trace ) { $.debug(" :: I/O write 3C4: value > 4"); }
			}
		...
		case 0x03C5: // Sequencer Registers 00 -> 04
			...
			case 4: // Sequencer: memory mode register
				...
				if ( trace ) {
					$.debug(" :: I/O write 0x3C5: memory mode:");
					$.debug(" -> extended_mem = " + state.sequencer.extended_mem);
					$.debug(" -> odd_even = " + state.sequencer.odd_even);
					$.debug(" -> chain_four = " + state.sequencer.chain_four);
				}
				break;
			default:
				if ( trace ) {
					$.debug(" :: I/O write 0x3C5: index "
		...
		case 0x03C6: // PEL mask
			...
			if ( state.pel.mask != 0xFF ) {
				if ( trace ) {
					$.debug(" :: I/O write 0x3C6: PEL mask="
						+ $.format("hex", val) + " != 0xFF");
				}
			}
		...
		case 0x03C9: // PEL Data Register, colors 00 -> FF
			...
			case 2:
				...
				if ( trace ) { $.warning("palette_change()"); }
		...
		case 0x03CE: // Graphics Controller Index Register
			if ( val > 0x08 ) { // [Bochs] ???
				if ( trace ) { $.debug(" :: I/O write: 0x3CE: value > 8"); }
			}
		...
		case 0x03CF: // Graphics Controller Registers 00 -> 08
			...
			case 5: // Mode
				...
				if ( trace ) {
					$.debug(" :: I/O write: 0x3CF: mode reg:"
						+ " value = " + $.format("hex", val));
				}
				if ( state.graphics_ctrl.odd_even ) {
					if ( trace ) { $.debug(" -> graphics_ctrl.odd_even = 1"); }
				}
				if ( state.graphics_ctrl.shift_reg ) {
					if ( trace ) { $.debug(" -> graphics_ctrl.shift_reg = 1"); }
				}
				break;
			case 6: // Miscellaneous
				...
				if ( trace ) {
					$.debug(" :: Miscellaneous:");
					$.debug(" -> memory_mapping set to ",
						+ state.graphics_ctrl.memory_mapping);
					$.debug(" -> graphics mode set to ",
						+ state.graphics_ctrl.graphics_alpha);
					$.debug(" -> odd_even mode set to ",
						+ state.graphics_ctrl.odd_even);
					$.debug(" :: I/O write: 0x3cf: misc reg: value = "
						+ $.format("hex", val));
				}
		...
		case 0x03D4: // CRTC Index Register (color emulation modes)
			...
			if ( state.CRTC.address > 0x18 ) {
				if ( trace ) {
					$.debug(" :: I/O write: invalid CRTC register "
		...
		case 0x03DA: // Feature Control (color emulation modes)
			if ( trace ) {
				$.debug(" :: I/O write: 0x3DA: ignoring:"
		...
		default:
		case 0x03C1: // [Bochs] ???
			$.problem(" :: Unsupported write, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
		
		if ( charmap_update ) {
			if ( trace ) { $.warning("set_text_charmap()"); }
