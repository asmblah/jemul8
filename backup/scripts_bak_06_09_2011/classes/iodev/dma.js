...
.module("iodev/dma", function ( $ ) { "use strict";
	...
	function DMA( machine ) {
		$.assert(this && (this instanceof DMA), "DMA ctor ::"
	...
	$.inherit(DMA, x86Emu.IODevice, "DMA"); // Inheritance
	...
	DMA.prototype.setDRQ = function ( idx_channel, val ) {
		...
		if ( idx_channel > 7 ) {
			$.panic("DMA.setDRQ() :: idx_channel > 7");
			return 0;
		}
		...
		channel.DRQ.set(val);
		if ( !channel.inUse ) {
			$.panic("DMA.setDRQ() :: Channel " + idx_channel + " not connected to device");
			return 0;
		}
		// ( See note above )
		idx_channel &= 0x03;
		...
		if ( (baseDMA & (0x7FFF0000 << isSlave))
			...
			// ( NB: isSlave will be converted to int )
			$.panic("DMA.setDRQ() :: Request outside "
	...
	DMA.prototype.registerDMA16Channel = function ( idx_channel, fnRead, fnWrite, name ) {
		if ( idx_channel < 4 || idx_channel > 7 ) {
			$.panic("DMA.registerDMA16Channel() ::"
				+ " Invalid channel #" + idx_channel);
			return false; // Fail
		}
		if ( this.DMA2.list_channel[ idx_channel & 0x03 ].inUse ) {
			$.panic("DMA.registerDMA16Channel() :: Channel #"
				+ idx_channel + " already in use");
			return false; // Fail
		}
		$.info("DMA.registerDMA16Channel() :: Channel #"
	...
	DMA.prototype.unregisterDMAChannel = function ( idx_channel ) {
		...
		$.info("DMA.unregisterDMAChannel() :: Channel #"
	...
	function readHandler( device, addr, io_len ) {
		...
		case 0xDE: // DMA-2 undocumented: read all mask bits
			...
			return (0xF0 | result8);
		default:
			$.problem("DMA readHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		default:
			$.problem("DMA writeHandler() :: Unsupported write, address=" + $.format("hex", addr) + "!");
		}
	}
	/* ====== /Private ====== */
