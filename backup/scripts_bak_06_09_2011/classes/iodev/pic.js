...
.module("iodev/pic", function ( $ ) { "use strict";
	...
	function PIC( machine ) {
		$.assert(this && (this instanceof PIC), "PIC ctor ::"
	...
	$.inherit(PIC, x86Emu.IODevice, "PIC"); // Inheritance
	...
	function readHandler( device, addr, io_len ) {
		...
		$.info("PIC readHandler() :: Read from "
		...
		case 0x00A1: // Read from Slave IMR "Interrupt Mask Register"
			...
			return result8;
		default:
			$.panic("PIC readHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		$.info("PIC writeHandler() :: Write to address: "
		...
		case 0xA1:
			...
			return;
		default:
			$.panic("PIC writeHandler() :: Unsupported write, address="
				+ $.format("hex", addr) + "!");
			return;
		}
	}
