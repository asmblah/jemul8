...
.module("iodev/guest2host", function ( $ ) { "use strict";
	...
	$.inherit(Guest2Host, x86Emu.IODevice, "Guest2Host"); // Inheritance
	...
	function readHandler( device, addr, io_len ) {
		...
		$.info("Guest2Host readHandler() :: Read from address: "
			+ $.format("hex", addr));
		...
		case 0x0080: // PORT_DIAG
			...
			break;
		default:
			$.panic("Guest2Host readHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
