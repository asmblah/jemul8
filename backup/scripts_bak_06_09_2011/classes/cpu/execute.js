...
.module("cpu/execute", function ( $ ) { "use strict";
	...
	var max_insn_perSlice = Math.floor(millisecsPerSlice
	...
	x86Emu.x86CPU.prototype.exec = function () {
		...
		var yieldManager = function () {
			...
			millisecsPauseForYield = millisecsYield - (machine.getTimeMsecs() - ticksYieldStart);
	...
	x86Emu.x86CPU.prototype.fetchDecodeExecute = function () {
		...
		for ( idx_insn = 0
		; idx_insn < max_insn_perSlice && !this.isHalted ; ++idx_insn ) {
			...
			// Instruction needs to be decoded into cache
				...
				if ( dataOpcode.length > 1 ) {
					...
					insn.mask_sizeOperand
						= x86Emu.generateMask(insn.sizeOperand);
				}
			...
			//if ( PC > 0xFFFF0 ) {
				//debugger;
			//}
		} // End of CPU loop => max_insn_perSlice
