	...
	x86Emu.x86CPU.prototype.getInstructions = function () {
		...
		//				by a conditional jump. (ie. should call CPU.ZF.set() etc.)
			...
			case "#REP": // Deliberate fall-thru; see below
				$.problem("Instruction.execute() :: CMPS - #REP invalid");
		...
		//		array methods, instead of a possibly slow loop over each individual byte
			...
			// Repeat CX times
			case "#REP":
				$.problem("Instruction.execute() :: SCAS - #REP invalid");
			// Repeat while Equal, max CX times
			case "#REPE":
				cx = CPU.CX.get();
				len = cx * sizeOperand;
