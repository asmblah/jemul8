...
.module("bitflag", function ( $ ) { "use strict";
	...
	function BitFlag( name, regMaster, bitsInLeft ) {
		/* ==== Guards ==== */
		$.assert(this != self, "BitFlag constructor ::"
	...
	//BitFlag.prototype = new Bit();
	$.inherit(BitFlag, Bit); // Inheritance
	...
	function BitFlag_CreateSetter( regMaster, bitsInLeft ) {
		...
		if ( bitsInLeft == 0 ) {
			return function () {
				/* ==== Guards ==== */
				$.assert(arguments.length === 0, "BitFlag.set :: Does not take any arguments (hint: just .set() to set=1 or .clear() to set=0).");
				/* ==== /Guards ==== */
				regMaster.set(
	...
	// Superclass for values represented by 1 binary bit
	function Bit() {
