...
.module("cpu/execute", function ( $ ) { "use strict";
	...
	jemul8.x86CPU.prototype.init = function () {
		...
		function yieldManager() {
			...
			window.setTimeout(yieldManager, 0);//msPauseForYield);
		}
	...
	jemul8.x86CPU.prototype.fetchDecodeExecute = function () {
		...
		// CPU's instruction cache (for speed... !)
			...
			, insns, max_insnsPerSlice = this.max_insnsPerSlice
			, tmr
			//, ticksEndSlice = machine.getTimeMsecs() + this.msPerSlice
			, ticksSlice = (1000 / 30)
		...
		do {
		// for ( insns = 0
		// ; insns < max_insnsPerSlice && !this.isHalted ; ++insns ) {
		...
		} while ( (insns % 200) || Date.now() <= ticksEndSlice );
		...
		++yps;
		ips += insns;
		if ( ticksNow > (ticksLastUpdate + 1000) ) {
			$("#performance").text("IPS: " + ips + ", YPS: " + yps);
