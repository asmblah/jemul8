...
.module("io", function ( $ ) { "use strict";
	...
	= function ( device, addr, name_portPart, fn, mask ) {
		/* ==== Guards ==== */
		jemul8.assert(device && (device instanceof jemul8.IODevice)
			, "IO.registerIO_Read() :: 'device' must be a valid IODevice");
		jemul8.assert(!isNaN(addr) && addr === parseInt(addr)
		...
		// IO port conflict
			...
			jemul8.problem("IO.registerIO_Read() :: IO port conflict - "
				+ $.format("hex", addr) + " already assigned as "
				+ port.name_port);
			return false;
		}
		
		jemul8.debug("IO.registerIO_Read() :: I/O read port "
	...
	= function ( device, addr, name_portPart, fn, mask ) {
		...
		// IO port conflict
			...
			jemul8.problem("IO.registerIO_Write() :: I/O port conflict - "
				+ $.format("hex", addr) + " already assigned as "
				+ port.name_port);
			return false;
		}
		
		jemul8.debug("IODevice.registerIO_Write() :: I/O write port "
			+ $.format("hex", addr) + " assigned as "
			+ name_port + " to " + device.name);
	...
	IO.prototype.registerIRQ = function ( device, irq, namePart ) {
		...
		jemul8.assert(!isNaN(irq) && irq === parseInt(irq)
		...
		if ( irq < 0 ) {
			return jemul8.panic("IO.registerIRQ() :: IO device '"
	...
	IO.prototype.unregisterIRQ = function ( device, irq, namePart ) {
		...
		// IRQ index out of bounds
		if ( irq < 0 ) {
			return jemul8.panic("IO.unregisterIRQ() :: IO device '"
		...
		jemul8.debug("IO.unregisterIRQ() :: IRQ #" + irq
	...
	function null_readHandler( device, addr, lenIO ) {
		jemul8.problem("I/O read from null handler (I/O port unassigned) - port "
			+ $.format("hex", addr));
	...
	function null_writeHandler( device, addr, val, lenIO ) {
		jemul8.problem("I/O write to null handler (I/O port unassigned) - port "
