...
.module("iodev/keyboard", function ( $ ) { "use strict";
	...
	function KeyboardCntrlr( machine ) {
		$.assert(this && (this instanceof KeyboardCntrlr)
			, "KeyboardCntrlr ctor ::"
		...
		$.info("KeyboardCntrlr (Intel 8042) PreInit");
	...
	KeyboardCntrlr.prototype = new x86Emu.IODevice( "KeyboardCntrlr", KeyboardCntrlr ); // Inheritance
	...
	KeyboardCntrlr.prototype.sendToKeyboard = function ( val ) {
		var keyboard = this.keyboard
			, cps;
		$.debug("KeyboardCntrlr.sendToKeyboard() :: Controller passed byte "
		...
		if ( keyboard.bufInternal.expecting_typematic ) {
			...
			case 0: $.info("KeyboardCntrlr.sendToKeyboard() ::"
				+ " Setting delay to 250 mS (unused)"); break;
			case 1: $.info("KeyboardCntrlr.sendToKeyboard() ::"
				+ " setting delay to 500 mS (unused)"); break;
			case 2: $.info("KeyboardCntrlr.sendToKeyboard() ::"
				+ " setting delay to 750 mS (unused)"); break;
			case 3: $.info("KeyboardCntrlr.sendToKeyboard() ::"
			...
			$.info("KeyboardCntrlr.sendToKeyboard() :: Setting repeat rate"
				+ " to " + cps + " cps (unused)");
		...
		if ( keyboard.bufInternal.expecting_led_write ) {
			...
			$.debug("KeyboardCntrlr.sendToKeyboard() :: LED status set to "
		...
		if ( keyboard.state.expecting_scancodes_set ) {
			...
			if ( val !== 0 ) {
				if ( val < 4 ) {
					...
					$.info("KeyboardCntrlr.sendToKeyboard() ::"
						+ " Switched to scancode set"
						+ keyboard.state.current_scancodes_set + 1
					);
					keyboard.enqueueKey(0xFA); // Send (ACK)nowledge
				} else {
					$.problem("KeyboardCntrlr.sendToKeyboard() ::"
		...
		case 0xF0: // Select alternate scan code set
			...
			$.debug("KeyboardCntrlr.sendToKeyboard() ::"
		...
		case 0xFE:
			$.panic("KeyboardCntrlr.sendToKeyboard() ::"
				+ " Got 0xFE (resend) !!!");
			break;
	...
	KeyboardCntrlr.prototype.sendToMouse = function ( val ) {
		var keyboard = this.keyboard, mouse = this.mouse
			, cps;
		$.debug("KeyboardCntrlr.sendToMouse() :: Controller passed byte "
			+ $.format("hex", val) + " to mouse");
		
		/** NB: This is _always_ a PS/2 mouse **/
		
		$.debug(" -> enable = ", mouse.state.enable);
		$.debug(" -> allow_irq12 = " + keyboard.state.allow_irq12);
		$.debug(" -> aux_clock_enabled = " + keyboard.state.aux_clock_enabled);
		...
		if ( keyboard.state.expecting_mouse_parameter ) {
			...
			case 0xF3: // Set Mouse Sample Rate
				...
				$.debug("KeyboardCntrlr.sendToMouse() ::"
				...
				if ( (val === 200) && (!mouse.state.im_request) ) {
					...
					$.info("KeyboardCntrlr.sendToMouse() ::"
			...
			case 0xE8: // Set Mouse Resolution
				...
				case 3:
					...
					break;
				default:
					$.panic("KeyboardCntrlr.sendToMouse() ::"
			...
			case 0xE6: // Set Mouse Scaling to 1:1
				...
				$.debug("KeyboardCntrlr.sendToMouse() :: Scaling set to 1:1");
				break;
			
			case 0xE7: // Set Mouse Scaling to 2:1
				mouse.enqueueCntrlr(0xFA); // (ACK)nowledge
				mouse.state.scaling = 2;
				$.debug("KeyboardCntrlr.sendToMouse() :: Scaling set to 2:1");
				break;
			...
			case 0xEA: // Set Stream Mode
				$.debug("KeyboardCntrlr.sendToMouse() :: Stream mode on");
			...
			case 0xEC: // Reset Wrap Mode
				...
				if ( mouse.state.mode === MOUSE_MODE_WRAP ) {
					$.debug("KeyboardCntrlr.sendToMouse() :: Wrap mode off");
			...
			case 0xEE: // Set Wrap Mode
				...
				$.debug("KeyboardCntrlr.sendToMouse() :: Wrap mode on");
			...
			case 0xF0: // Set Remote Mode (polling mode, i.e. not stream mode.)
				$.debug("KeyboardCntrlr.sendToMouse() :: Remote mode on");
			...
			case 0xF2: // Read Device Type
				...
				$.debug("KeyboardCntrlr.sendToMouse() :: Read mouse ID");
				break;
			...
			case 0xF6: // Set Defaults
				...
				mouse.state.mode            = MOUSE_MODE_STREAM;
				mouse.enqueueCntrlr(0xFA); // (ACK)nowledge
				$.debug("KeyboardCntrlr.sendToMouse() :: Set defaults");
				break;
			...
			case 0xFF: // Reset
				...
				$.debug("KeyboardCntrlr.sendToMouse() :: Mouse reset");
				break;
			...
			case 0xEB: // Read Data (send a packet when in Remote Mode)
				...
				$.problem("KeyboardCntrlr.sendToMouse() :: Warning:"
					+ " Read Data command partially supported.");
				break;
			
			case 0xBB: // OS/2 Warp 3 uses this command
				$.problem("KeyboardCntrlr.sendToMouse() ::"
	...
	Keyboard.prototype.enqueueCntrlr = function ( data ) {
		...
		$.debug("KeyboardCntrlr.enqueueCntrlr(" + $.format("hex", data) + ")");
		
		// See if we need to queue this byte from the controller
		if ( state.outb ) {
			if ( state.sizeQueue >= KBD_CONTROLLER_QSIZE ) {
				$.panic("KeyboardCntrlr.enqueueCntrlr(" + $.format("hex", data)
					+ ") :: state.queue full!");
			}
	...
	Keyboard.prototype.enqueueKey = function ( scancode ) {
		...
		if ( !this.state.outb && this.state.kbd_clock_enabled ) {
			this.cntrlr.activateTimer();
			$.debug("Keyboard.enqueueKey(...) :: Activating timer...");
			return;
		}
	};
	...
	function Mouse( cntrlr ) {
		...
		this.state = {
			...
			, get_status_byte: function () {
				...
				default:
					$.panic("Mouse.state.get_resolution_byte() ::"
	...
	Mouse.prototype.enqueueCntrlr = function ( data ) {
		...
		$.debug("Mouse.enqueueCntrlr(" + $.format("hex", data) + ")");
		
		// See if we need to queue this byte from the controller
		//	(even for mouse bytes)
		if ( state.outb ) {
			if ( state.sizeQueue >= KBD_CONTROLLER_QSIZE ) {
				$.panic("Mouse.enqueueCntrlr(" + $.format("hex", data)
					+ ") :: state.queue full!");
			}
			state.queue[ state.sizeQueue++ ] = data;
			state.sourceQueue = 1;
	...
	Mouse.prototype.enqueueData = function ( data ) {
		...
		if ( !this.cntrlr.state.outb && this.cntrlr.state.aux_clock_enabled ) {
			this.cntrlr.activateTimer();
			$.debug("Mouse.enqueueData(...) :: Activating timer...");
			return;
		}
	};
	...
	function readHandler( device, addr, len ) {
		...
		//$.info("KeyboardCntrlr readHandler() :: Read addr = "
		//	+ $.format("hex", addr));
		...
		case 0x60: // Output buffer
			...
			if ( state.auxb ) {
				...
				$.debug("KeyboardCntrlr (for aux - mouse) readHandler() ::"
			...
			// Keyboard byte available
				...
				if ( state.sizeQueue ) {
					...
					$.debug("KeyboardCntrlr readHandler() ::"
			...
			$.debug("num_elements = %d", keyboard.bufInternal.num_elements);
			$.debug("KeyboardCntrlr readHandler() ::"
				+ " Read from " + $.format("hex", addr)
				+ " with .outb empty");
		...
		default:
			$.problem("KeyboardCntrlr readHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	function writeHandler( device, addr, val, len ) {
		...
		$.info("KeyboardCntrlr writeHandler() :: 8-bit write to address: "
		...
		case 0x60: // Input buffer
			...
			if ( state.expecting_port60h ) {
				...
				if ( state.inpb ) {
					$.problem("KeyboardCntrlr writeHandler() ::"
				...
				case 0x60: // Write command byte
					...
					$.debug("KeyboardCntrlr writeHandler() :: allow_irq12 set to "
						+ state.allow_irq12);
					
					if ( !scan_convert ) {
						$.info("KeyboardCntrlr writeHandler() ::"
				...
				case 0xCB: // Write keyboard controller mode
					$.debug("KeyboardCntrlr writeHandler() ::"
				...
				case 0xD2:
					...
					break;
				default:
					$.panic("KeyboardCntrlr writeHandler() ::"
		...
		case 0x64: // Control register
			...
			case 0xAD: // Disable keyboard
				...
				$.debug("KeyboardCntrlr writeHandler() :: Keyboard disabled");
				break;
			case 0xAE: // Enable keyboard
				keyboard.setClockEnabled(true);
				$.debug("KeyboardCntrlr writeHandler() :: Keyboard enabled");
				break;
			...
			break;
		default:
			$.problem("KeyboardCntrlr writeHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	// Exports
	x86Emu.KeyboardCntrlr = KeyboardCntrlr;
});
