...
.module("cpu/operand", function ( $ ) { "use strict";
	...
	function Operand( insn, offset, bytOpcode, nameMapFlags
		...
		/* ==== Guards ==== */
		$.assert(this && (this instanceof Operand), "Operand ctor ::"
			+ " error - constructor not called properly");
		/* ==== /Guards ==== */
		
		var machine = insn.machine, cpu = machine.cpu, dram = machine.dram
			, addrMethodCode
			, addrMethod
			, displacement
			, length = 2
		...
		if ( typeof nameMapFlags === "number" ) {
			...
			if ( (nameMapFlags & 0xFF00) !== 0x1B00 ) {
				...
				// Use a fast switch to decide how to proceed
					...
					case 0x0100: //"A":
						this.setImmediate(dram.readBytes(this.offset, length));
					...
					case 0x0F00: //"O":
						this.setDisplacement(dram.readBytes(this.offset
							, insn.sizeOperand));
						// Move offset pointer past the value just read
						...
						// 16-bit addressing mode
							...
							if ( fieldMod == 0x00 ) {
								...
								if ( fieldRM === 0x06 ) {
									...
									displacement = dram.readBytes(this.offset, 2);
								...
								this.setType("GENERAL");
								this.length = this.reg.sizeBytes;
					...
					case 0x0600: //"F":
						break;
					default:
						$.problem("Invalid AddressingMethodCode '" + addrMethodCode + "'.");
					}
				}
				/* ============ /Determine addressing method from AddressMethodCode ============ */
		...
		} else if ( length == 2 && (this.reg = cpu.hsh_reg[ nameMapFlags.substr(1) ]) ) {
			this.setType("GENERAL");
			this.length = this.reg.sizeBytes;
			this.mask = generateMask(this.length);
		// Flag indicates a 32-bit general purpose register ( eg. EAX, ESI )
		} else if ( length == 4 && (this.reg = cpu.hsh_reg[ nameMapFlags.toUpperCase() ]) ) {
			this.setType("GENERAL");
			this.length = this.reg.sizeBytes;
			this.mask = generateMask(this.length);
		}
	...
	Operand.prototype.signExtend = function ( sizeTo ) {
		return this.insn.machine.cpu.signExtend(this.read(), this.length
			, sizeTo || this.insn.operand1.length);
	};
	...
	/*hsh_func_operandRead_WithPointer.SEGMENT = */hsh_func_operandRead_WithPointer.GENERAL = function () {
		var res = this.insn.machine.dram.readBytes((this.getSegment() << 4) + ((this.reg.get() + (this.reg2 ? this.reg2.get() : 0) + this.displacement) /* * this.insn.sizeAddress*/), this.length);
		return res;
	};
	// Direct memory address/displacement; no register involved
	hsh_func_operandRead_WithPointer.MEM_DISPLACEMENT = function () {
		var res = this.insn.machine.dram.readBytes((this.getSegment() << 4) + (this.displacement /* * this.insn.sizeAddress*/), this.length);
		return res; 
	};
	
	hsh_func_operandWrite_NonPointer.SEGMENT = hsh_func_operandWrite_NonPointer.GENERAL = function ( val ) {
		if ( val > this.mask || val > 0xFFFF ) { debugger; }
	...
	hsh_func_operandWrite_WithPointer.SEGMENT = hsh_func_operandWrite_WithPointer.GENERAL = function ( val ) {
		if ( (val > this.mask || val > 0xFFFF) && !this.isPointer ) { debugger; }
		
		this.insn.machine.dram.writeBytes((this.getSegment() << 4) + ((this.reg.get() + this.displacement) /* * this.insn.sizeAddress*/), val & this.mask, this.length);
	};
	// Direct Memory Address/displacement; no register involved
	hsh_func_operandWrite_WithPointer.MEM_DISPLACEMENT = function ( val ) {
		if ( val > this.mask || val > 0xFFFF ) { debugger; }
		
		this.insn.machine.dram.writeBytes((this.getSegment() << 4) + (this.displacement /* * this.insn.sizeAddress*/), val & this.mask, this.length);
	};
	
	// Create a bitmask for the specified value size in bytes
	//	(eg. for masking off higher bits of a value to fit it
	//	into a CPU register)
	function generateMask( size /* in bytes */ ) {
