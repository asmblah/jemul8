...
.module("cpu/instruction", function ( $ ) { "use strict";
	...
	function Instruction( machine, offset, name
		...
		/* ==== Guards ==== */
		$.assert(this && (this instanceof Instruction), "Instruction ctor ::"
	...
	x86Emu.x86CPU.prototype.getInstructions = function () {
		...
		// Adjusted Requested Privilege Level of Selector ( 286+ Protected Mode )
			...
			$.problem("Execute (ARPL) :: No Protected Mode support yet.");
			return;
		...
		//	Based on http://siyobik.info/index.php?module=x86&id=18
			...
			$.problem("Execute (BOUND) :: No Array bounds support yet.");
		...
		//	( other code segment / intersegment call )
			...
			$.problem("Execute (CALLF_AI) :: unsupported");
		...
		// Convert Double to Quad ( 386+ )
			...
			$.problem("Execute (CDQ) :: unsupported");
		// Clear Carry flag
		}, "CLC": function () {
			CPU.CF.clear();
		...
		//				by a conditional jump. ( ie. should call CPU.ZF.set() etc. )
			...
			case "#REPE":	// For CMPS, it would make little sense to use REP CMPS ( ... ),
				...
				if ( CPU.DF.get() ) {
					...
					while ( --len ) {
						...
						CPU.ESI.set(
							idxESI = (idxESI - sizeOperand)
						);
						CPU.EDI.set(
							idxEDI = (idxEDI - sizeOperand)
						);
			...
			case "#REPNE":
				...
				break;
			default:
				$.problem("Execute (CMPS) :: invalid string repeat operation/prefix.");
			}
		// Compare and Exchange (486+)
		}, "CMPXCHG": function () {
			var reg_acc = CPU.accumulator[ this.operand1.length ]
				, val_acc = reg_acc.get()
				, val1 = this.operand1.signExtend()
		...
		// Convert Word to Extended Dword ( 386+ )
			...
			$.problem("Execute (CWDE) :: unsupported");
		// Decimal Adjust after Addition
		}, "DAA": function () {
			$.problem("Execute (DAA) :: unsupported");
		// Decimal Adjust for Subtraction
		}, "DAS": function () {
			debugger;
			$.problem("Execute (DAS) :: unsupported");
		// Decrement
		}, "DEC": function () {
		...
		// Escape
			...
			$.problem("Execute (ESC) :: unsupported");
		// Halt CPU
		...
		// Input Byte or Word from Port
			...
			if ( port.mask & lenIO ) {
				...
				if ( addr_port !== 0x0CF8 ) {
					debugger;
					$.problem("Execute (IN) :: Read from port "
		...
		// Input String from Port ( 80188+ )
			...
			$.problem("Execute (INS) :: Not implemented yet");
		...
		// Invalidate Cache ( 486+ )
			...
			$.problem("Execute (INVD) :: unsupported");
		// Invalidate Translation Look-Aside Buffer Entry ( 486+ )
		}, "INVLPG": function () {
			$.problem("Execute (INVLPG) :: unsupported");
		...
		//	( NB: this conditional jump has no inverse )
			...
			// Quickly skip if condition not met
			if ( (this.sizeOperand === 2 // Base on operand-size attr
				? CPU.CX : CPU.ECX).get() !== 0 ) { return; }
		...
		// Load Access Rights Byte
			...
			$.problem("Execute (LAR) :: unsupported");
		...
		// Load Global Descriptor Table Register
			...
			$.problem("Execute (LGDT) :: unsupported");
		// Load Interrupt Descriptor Table Register
		}, "LIDT": function () {
			$.problem("Execute (LIDT) :: unsupported");
		...
		// Load Local Descriptor Table Register
			...
			$.problem("Execute (LLDT) :: unsupported");
		// Load Machine Status Word
		}, "LMSW": function () {
			$.problem("Execute (LMSW) :: unsupported");
			// CPU.CR0
		...
		// Load Segment Limit
			...
			$.problem("Execute (LSL) :: unsupported");
		// Load Task Register
		}, "LTR": function () {
			$.problem("Execute (LTR) :: unsupported");
		// Move ( Copy ) data
		}, "MOV": function () {
			...
			this.operand1.write(this.operand2.read() & this.operand2.mask);
		...
		//		array methods, instead of a possibly slow loop over each individual byte
			...
			case "#REPNE":
				...
				break;
			default:
				$.problem("Execute (MOVS) :: invalid string repeat operation/prefix.");
			}
		...
		// Output to Port
			...
			var addr_port = this.operand1.read()
				...
				, lenIO = this.operand2.length
			// All ports are initialised with null handlers,
			//	so there will always be a valid port object available
				, port = machine.hsh_portIOWrite[ addr_port ];
			...
			// Don't flood the logs when probing PCI (from Bochs)
				...
				debugger;
				$.problem("Execute (OUT) :: Write to port "
		...
		// Output String to Port
			...
			$.problem("Execute (OUTS) :: Not implemented yet");
		...
		// Rotate Bits Left with Carry Flag
			...
			$.problem("Execute (RCL) :: unsupported");
		// Rotate Bits Right with Carry Flag
		}, "RCR": function () {
			$.problem("Execute (RCR) :: unsupported");
		...
		//		array methods, instead of a possibly slow loop over each individual byte
			...
			case "#REPNE":
				...
				break;
			default:
				$.problem("Execute (SCAS) :: invalid string repeat operation/prefix.");
			}
		...
		// Store Global Descriptor Table Register
			...
			$.problem("Execute (SGDT) :: unsupported");
		// Store Interrupt Descriptor Table Register
		}, "SIDT": function () {
			$.problem("Execute (SIDT) :: unsupported");
		// Shift Left - Double Precision
		}, "SHLD": function () {
			$.problem("Execute (SHLD) :: unsupported");
		// Shift Right - Double Precision
		}, "SHRD": function () {
			$.problem("Execute (SHRD) :: unsupported");
		// Store Local Descriptor Table Register
		}, "SLDT": function () {
			$.problem("Execute (SLDT) :: unsupported");
		...
		// Store Task Register
			...
			$.problem("Execute (STR) :: unsupported");
		// Logical Compare
		}, "TEST": function () {
			var val1 = this.operand1.read();
		...
		// Verify a Segment for Reading
			...
			$.problem("Execute (VERR) :: unsupported");
		// Verify a Segment for Writing
		}, "VERW": function () {
			$.problem("Execute (VERW) :: unsupported");
	...
	function jumpShortOrNear( insn ) {
		...
		if ( op1.length === 1 ) {
			...
			val = cpu.IP.get() + op1.signExtend(2);
	...
	// Exports
	x86Emu.Instruction = Instruction;
});
