...
var mod = new jemul8.SecondaryModule( function ( jemul8, machine, CPU, DRAM ) {
	...
	x86Emu.x86CPU.prototype.fetchDecodeExecute = function () {
		var idx, list, len
		//	, bitD_ProtectedMode_SegmentDesc = 0;
		// Size of memory addresses
			, sizeAddress_Bytes
			, sizeAddress_Bytes_WithPrefix
		// Size of operands
			, sizeOperand_Bytes
			, sizeOperand_Bytes_WithPrefix
		...
		for ( idx_insn = 0 ; idx_insn < num_max_insnPerSlice && !this.isHalted() ; ++idx_insn ) {
			...
			if ( 1||!(insn = arr_insnCache[ offset ]) ) {
				...
				insn = arr_insnCache[ offsetStart ] = new Instruction( offsetStart, nameInstruction, sizeAddress_Bytes, sizeOperand_Bytes );
			...
			//if ( this.EIP.get() == 0x7D4D ) {
			//if ( this.EIP.get() == 0x7CC4 ) {
				//return;//
				//debugger;
				//this.halt();
				//return;
