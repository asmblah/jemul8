...
.module("main", function ( $ ) {
	...
	function x86Emu() {
		...
		// Emulated IBM-compatible PC
		this.machine = machine = new x86Emu.x86IBM_PC( this );
		// Set up the I/O device subsystem as early as possible
		this.IODevice.init();
		// Emulated x86 IA_32 CPU
		//	( based on an Intel 80486 / i486 )
		machine.cpu = cpu = new x86Emu.x86CPU( this, "486" );
		// DRAM Controller
		machine.dram = dram = new x86Emu.x86DRAM( this );
		// Emulated BIOS
		//	( based on Phoenix BIOS )
		machine.bios = bios = new x86Emu.x86BIOS( "Phoenix / jemul8" );
		
		// IO devices
		machine.cmos = new x86Emu.CMOS( this );
		machine.pic = new x86Emu.PIC( this );
		machine.dma = new x86Emu.DMA( this );
		...
		machine.keyboard = keyboard = new x86Emu.Keyboard();
		...
		this.speakerOnboard = new x86Emu.Speaker();
		
		// Create & Install first available Floppy drive, floppy0
		machine.floppy0 = new x86Emu.FloppyDrive( 0 );
		...
		plugin.loadDeferredModules(/*machine, CPU, DRAM*/);
	}
	// Start the emulator!
