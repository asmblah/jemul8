...
var mod = new jemul8.PrimaryModule( function ( jemul8 ) {
	...
	function x86BIOS( name ) {
		...
		this.wordEquipmentList = new jemul8.Bitfield( 16 );
		
		// Download & store BIOS firmware image
		this.memData = jemul8.getSyncHTTP_Binary("docs/bochs-20100605/bios/BIOS-bochs-legacy");
	}
	...
	x86BIOS.prototype.Prepare = function () {
		 ...
		 */
		var cache_segment = 0;//0xFFF00000;
		var segment = CPU.CS.get(); // Should be 0xF000;
		...
		var addrFlat = (cache_segment | (segment << 4) + offset) >>> 0;
		var memData = this.memData, memData_DRAM = DRAM.memData, idx, len = memData.length;
