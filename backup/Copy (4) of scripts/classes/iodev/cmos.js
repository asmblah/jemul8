...
.module("emulator", function ( $ ) {
	...
	CMOS.prototype = new jemul8.IODevice( "CMOS", CMOS ); // Inheritance
	...
	function ReadHandler( device, addr, len ) {
		...
		jemul8.Info("CMOS ReadHandler() :: Read of CMOS register " + jemul8.HexFormat(state.addrMemory));
		//debugger;
		switch ( addr ) {
		case 0x0070:
		...
		case 0x0071:
			...
			return result8;
		default:
			jemul8.Panic("CMOS ReadHandler() :: Unsupported read, address=" + jemul8.HexFormat(addr) + "!");
			return 0;
		}
	}
	...
	function WriteHandler( device, addr, val, len ) {
		...
		var dcc;
		var valCRBPrevious;
		jemul8.Info("CMOS WriteHandler() :: Write to address: " + jemul8.HexFormat(addr) + " = " + jemul8.HexFormat(val));
		//debugger;
		switch ( addr ) {
		...
		case 0x0071:	// Write to current register
			...
			case REG_HOUR_ALARM:
				...
				jemul8.Debug("CMOS WriteHandler() :: Alarm time changed to " + jemul8.TimeFormat(state.regHourAlarm.get(), state.regMinAlarm.get(), state.regSecAlarm.get()));
				break;
			case REG_SEC:
			case REG_MIN:
			case REG_HOUR:
			...
			case REG_STAT_A: // Control Register A
				...
				if ( (dcc & 0x06) == 0x06 ) {
					...
					jemul8.Panic("CRA: divider chain control " + jemul8.HexFormat(dcc));
				}
			...
			case REG_STAT_B: // Control Register B
				...
				if ( val & 0x01 ) {
					...
					jemul8.Error("CMOS WriteHandler() :: Write status reg B, daylight savings unsupported");
				}
			...
			case REG_STAT_D: // Control Register D
				jemul8.Error("CMOS WriteHandler() :: Write to control register " + jemul8.HexFormat(state.addrMemory) + " ignored (read-only)");
				break;
			case REG_DIAGNOSTIC_STATUS:
				jemul8.Debug("CMOS WriteHandler() :: Write register 0x0e: " + jemul8.HexFormat(val));
			...
			case REG_SHUTDOWN_STATUS:
				...
				case 0x0c: // RETF to DWORD pointer at 40:67
					...
					break;
				default:
					jemul8.Error("CMOS WriteHandler() :: Unsupported shutdown status: " + jemul8.HexFormat(val) + "!");
				}
				state.regShutdownStatus.set(val);
				break;
			default:
				jemul8.Debug("CMOS WriteHandler() :: Write reg 0x%02x: value = " + jemul8.HexFormat(val));
				state.list_reg[ state.addrMemory ].set(val);
			}
			break;
		default:
			jemul8.Panic("CMOS IO WriteHandler :: Unsupported write, address=" + jemul8.HexFormat(addr) + "!");
			return 0;
		}
	}
	/* ====== /Private ====== */
