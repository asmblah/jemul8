...
.module("emulator", function ( $ ) {
	...
	PIC.prototype = new jemul8.IODevice( "PIC", PIC ); // Inheritance
	...
	PIC.prototype.LowerIRQ = function ( irq ) {
		...
		if ( (irq <= 7) && (state.PICmaster.inIRQ & mask) ) {
			jemul8.Debug("PIC.LowerIRQ() :: IRQ line #" + irq + " (master PIC) now low");
		...
		// Slave PIC handles IRQs 8 -> 15
			...
			jemul8.Debug("PIC.LowerIRQ() :: IRQ line #" + irq + " (slave PIC) now low");
	...
	PIC.prototype.RaiseIRQ = function ( irq ) {
		...
		// Master PIC handles IRQs <= 7
		if ( (irq <= 7) && !(state.PICmaster.inIRQ & mask) ) {
			jemul8.Debug("PIC.RaiseIRQ() :: IRQ line #" + irq + " (master PIC) now high");
			state.PICmaster.inIRQ |= mask; // Set bit
			state.PICmaster.regInterruptRequest.set(state.PICmaster.regInterruptRequest.get() | mask);
			state.PICmaster.Service();
		// Slave PIC handles IRQs 8 -> 15
		} else if ( (irq > 7) && (irq <= 15) && !(state.PICslave.inIRQ & mask) ) {
			jemul8.Debug("PIC.RaiseIRQ() :: IRQ line #" + irq + " (slave PIC) now high");
	...
	PIC.prototype.setMode = function ( forMaster, mode ) {
		...
		jemul8.Debug("PIC(" + PIC.name + ").setMode() :: Setting mode (edge/level) to " + jemul8.HexFormat(mode));
		PIC.modeIRQ = mode;
	};
	PIC.prototype.AcknowledgeInterrupt = function () {
		var state = this.state, PICmaster = state.PICmaster, PICslave = state.PICslave;
		...
		if ( !(PICmaster.modeIRQ & (1 << PICmaster.irq)) ) {
			PICmaster.regInterruptRequest.set(PICmaster.regInterruptRequest.get() & (~(1 << PICmaster.irq)));
		}
		// In Auto-EOI mode, don't set the ISR bit
		if ( !PICmaster.useAutoEOI ) {
			PICmaster.regInService.set(PICmaster.regInService.get() | (1 << PICmaster.irq));
		...
		// Handled by slave PIC ( IRQ2, slave PIC IRQs 8 -> 15 )
			...
			PICslave.INT.Lower(); // Acknowledge that IRQ from slave has been rec'vd
			PICmaster.inIRQ &= ~(1 << 2);
			...
			if ( !(PICslave.modeIRQ & (1 << PICslave.irq)) ) {
				PICslave.regInterruptRequest.set(PICslave.regInterruptRequest.get() & (~(1 << PICslave.irq)));
			}
			// In Auto-EOI mode, don't set the ISR bit
			if ( !PICslave.useAutoEOI ) {
				PICslave.regInService.set(PICslave.regInService.get() | (1 << PICslave.irq));
	...
	function PIC_Core( isMaster ) {
		...
		this.doRotateOnAutoEOI = false; // Dictates when to rotate in Auto-EOI mode
		this.modeIRQ = 0; // IRQ mode bitmap (bit #0=edge, bit #1=level)
	}
	PIC_Core.prototype.InitCommand = function ( val ) {
		// Initialization command 1
		if ( val & 0x10 ) {
			jemul8.Debug("PIC(" + this.name + ") WriteHandler() :: Init command 1 found");
			jemul8.Debug("	requires 4 = %u" + (val & 0x01));
			jemul8.Debug("	cascade mode: [0=cascade, 1=single] " + ((val & 0x02) >> 1));
			this.Init.inInit = 1;
			...
			if ( val & 0x02 ) { return jemul8.Panic("PIC(" + this.name + ") WriteHandler() :: ICW1: single mode not supported"); }
			if ( val & 0x08 ) {
				return jemul8.Panic("PIC(" + this.name + ") WriteHandler() :: ICW1: level sensitive mode not supported");
			} else {
				jemul8.Debug("PIC(" + this.name + ") WriteHandler() :: ICW1: edge triggered mode selected");
			}
			CPU.INTR.Lower();
			return;
		}
		...
		case 0x67: // Specific EOI 7
			this.regInService.set(this.regInService.get() & (~(1 << (val - 0x60))));
			this.Service();
			break;
		...
		case 0xC7: // 7 6 5 4 3 2 1 0
			jemul8.Info("PIC(" + this.name + ") WriteHandler() :: IRQ lowest command " + jemul8.HexFormat(val));
		...
		case 0xE7: // Specific EOI and rotate 7
			this.regInService.set(this.regInService.get() & (~(1 << (val - 0xE0))));
		...
		case 0x02: // Single mode bit: 1 = single, 0 = cascade
			...
			break;
		default:
			return jemul8.Panic("PIC(" + this.name + ") WriteHandler() :: Write to port A0h = " + jemul8.HexFormat(val));
		}
	};
	PIC_Core.prototype.InitModeOperation = function ( val ) {
		if ( PICmaster.Init.inInit ) {
			...
			case 2:
				...
				jemul8.Debug("PIC(" + this.name + ") WriteHandler() :: Init command 2 = " + jemul8.HexFormat(val));
				jemul8.Debug("	offset = INT " + jemul8.HexFormat(PICmaster.offsetInterrupt));
				break;
			case 3:
				jemul8.Debug("PIC(" + this.name + ") WriteHandler() :: Init command 3 = " + jemul8.HexFormat(val));
				...
				break;
			case 4:
				jemul8.Debug("PIC(" + this.name + ") WriteHandler() :: Init command 4 = " + jemul8.HexFormat(val));
				if ( val & 0x02 ) {
				...
				break;
			default:
				return jemul8.Panic("PIC(" + this.name + ") WriteHandler() :: Expecting bad init command");
			}
			return;
		}
		
		// Normal operation
		jemul8.Debug("PIC(" + this.name + ") WriteHandler() :: Setting PIC IMR to " + jemul8.HexFormat(val));
	...
	PIC_Core.prototype.Service = function () {
		...
		// Normal mode
			...
			if ( valISR = this.regInService.get() ) {
				...
				if ( irqMax > 7 ) { return jemul8.Panic("PIC(" + this.name + ").Service() :: Error, irqMax > 7"); }
			} else {
			...
			do {
				...
				if ( !(this.useSpecialMask && ((this.regInService.get() >> irq) & 0x01)) ) {
					if ( unmasked_requests & (1 << irq) ) {
						jemul8.Debug("PIC(" + this.name + ").Service() :: Signalling IRQ #" + jemul8.HexFormat(this.isMaster ? irq : 8 + irq));
						this.INT.Raise();
						this.irq = irq;
						if ( this.isMaster ) {
							CPU.INTR.Raise();
	...
	function ReadHandler( device, addr, len ) {
		...
		jemul8.Info("PIC ReadHandler() :: Read from " + jemul8.HexFormat(state.addrMemory));
		...
		case 0x0020: // Read from Master "read register"
			...
			jemul8.Debug("PIC(master) ReadHandler() :: Read Master " + state.PICmaster.regReadSelect.name + " = " + jemul8.HexFormat(result8));
			return result8;
		case 0x0021: // Read from Master IMR "Interrupt Mask Register"
			result8 = state.PICmaster.regInterruptMask.get();
			jemul8.Debug("PIC(master) ReadHandler() :: Read Master IMR = " + jemul8.HexFormat(result8));
			return result8;
		case 0x00A0: // Read from Slave "read register"
			result8 = state.PICslave.regReadSelect.get();
			jemul8.Debug("PIC(slave) ReadHandler() :: Read Master " + state.PICslave.regReadSelect.name + " = " + jemul8.HexFormat(result8));
			return result8;
		case 0x00A1: // Read from Slave IMR "Interrupt Mask Register"
			result8 = state.PICslave.regInterruptMask.get();
			jemul8.Debug("PIC(slave) ReadHandler() :: Read Master IMR = " + jemul8.HexFormat(result8));
			return result8;
		default:
			return jemul8.Panic("PIC ReadHandler() :: Unsupported read, address=" + jemul8.HexFormat(addr) + "!");
		}
	}
	...
	function WriteHandler( device, addr, val, len ) {
		...
		jemul8.Info("PIC WriteHandler() :: Write to address: " + jemul8.HexFormat(addr) + " = " + jemul8.HexFormat(val));
