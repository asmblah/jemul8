...
var mod = new jsEmu.PrimaryModule( function ( jsEmu ) {
	...
	jsEmu.x86CPU.prototype.InstallStandardRegisters = function () {
		...
		this.InstallComponent(new jsEmu.LazyFlagRegister( "EFLAGS", 2 ));
		...
		// Unknown(1) Flag
		this.UF1 = this.hsh_reg["UF1"] = new LazyFlag( "UF1", this.EFLAGS, 1 );
		// Parity Flag
		this.PF = this.hsh_reg["PF"] = new LazyFlag( "PF", this.EFLAGS, 2 );
		// Unknown(2) Flag
		this.UF2 = this.hsh_reg["UF2"] = new LazyFlag( "UF2", this.EFLAGS, 3 );
		// Auxiliary Flag
		this.AF = this.hsh_reg["AF"] = new LazyFlag( "AF", this.EFLAGS, 4 );
		// Unknown(3) Flag
		this.UF3 = this.hsh_reg["UF3"] = new LazyFlag( "UF3", this.EFLAGS, 5 );
		// Zero Flag
		...
		this.TF = this.hsh_reg["TF"] = new LazyFlag( "TF", this.EFLAGS, 8 );
		// Interrupt Flag
		this.IF = this.hsh_reg["IF"] = new LazyFlag( "IF", this.EFLAGS, 9 );
		// Direction Flag
		this.DF = this.hsh_reg["DF"] = new LazyFlag( "DF", this.EFLAGS, 10 );
		// Overflow Flag
		this.OF = this.hsh_reg["OF"] = new LazyFlag( "OF", this.EFLAGS, 11 );
		// IOPL ( I/O Privilege Level ) Flag - Intel 286+ only
		this.IOPL = this.hsh_reg["IOPL"] = new LazyFlag( "IOPL", this.EFLAGS, 12 );
		this.IOPL2 = this.hsh_reg["IOPL2"] = new LazyFlag( "IOPL2", this.EFLAGS, 13 );
		// NT ( Nested Task ) Flag - Intel 286+ only
		this.NT = this.hsh_reg["NT"] = new LazyFlag( "NT", this.EFLAGS, 14 );
		// Unknown(4) Flag
		this.UF4 = this.hsh_reg["UF4"] = new LazyFlag( "UF4", this.EFLAGS, 15 );
		// Resume Flag
		this.RF = this.hsh_reg["RF"] = new LazyFlag( "RF", this.EFLAGS, 16 );
		// Virtual-8086 Mode Flag
		this.VM = this.hsh_reg["VM"] = new LazyFlag( "VM", this.EFLAGS, 17 );
		// Alignment-Check ( 486SX+ only )
		this.AC = this.hsh_reg["AC"] = new LazyFlag( "AC", this.EFLAGS, 18 );
		// Virtual Interrupt Flag ( Pentium+ )
		this.VIF = this.hsh_reg["VIF"] = new LazyFlag( "VIF", this.EFLAGS, 19 );
		// Virtual Interrupt Pending Flag ( Pentium+ )
		this.VIP = this.hsh_reg["VIP"] = new LazyFlag( "VIP", this.EFLAGS, 20 );
		// Identification Flag ( Pentium+ )
		this.ID = this.hsh_reg["ID"] = new LazyFlag( "ID", this.EFLAGS, 21 );
		...
		// Debug Control register contains 00000400h, disabling all breakpoint monitoring
