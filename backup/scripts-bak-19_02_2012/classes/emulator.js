...
.module("emulator", function ( $ ) { "use strict";
	...
	function jemul8( options ) {
		jemul8.assert(this && (this instanceof jemul8), "jemul8 ctor ::"
		...
		this.settings = new jemul8.Settings( defaults );
		...
		// Emulated IBM-compatible PC
		this.machine = machine = new jemul8.PC( this );
		// The memory subsystem
		machine.mem = new jemul8.Memory( machine );
		// The I/O subsystem
		machine.io = new jemul8.IO( machine );
		// Emulated x86 IA_32 CPU
		//	(based on an Intel 80486 / i486)
		machine.cpu = cpu = new jemul8.x86CPU( machine, "486" );
		// DRAM Controller
		//machine.dram = dram = new jemul8.x86DRAM( machine );
		...
		// IO devices
		machine.cmos = new jemul8.CMOS( machine );
		machine.dma = new jemul8.DMA( machine ); /*** NB: DMA module not finished yet! ***/
		machine.fdc = new jemul8.FDC( machine );
		machine.pic = new jemul8.PIC( machine );
		machine.pit = new jemul8.PIT( machine );
		machine.keyboard = new jemul8.KeyboardCntrlr( machine );
		// Small onboard speaker (eg. for BIOS POST beep codes)
		machine.speaker = new jemul8.Speaker( machine );
		machine.vga = new jemul8.VGA( machine ); // Video card
		// Special Guest<->Host interface for emulation
		machine.guest2host = new jemul8.Guest2Host( machine );
	...
	$.extend(jemul8.prototype, {
		getSetting: function ( name ) {
			...
			machine.reset(jemul8.PC.RESET_HARDWARE);
		}
	});
	...
	jemul8.prototype.reset = function () {
		this.pause();
		this.machine.reset(jemul8.PC.RESET_HARDWARE);
	};
	...
	this.data("jemul8", jemul8)
	// Expose constructor publicly
	.construct("jemul8", jemul8);
});
