...
.module("lazy_flag", function ( $ ) { "use strict";
	...
	function LazyFlag( name, regMaster, bitsInLeft ) {
		/* ==== Guards ==== */
		jemul8.assert(this && (this instanceof LazyFlag), "LazyFlag ctor ::"
		...
		default:
			jemul8.problem("LazyFlag constructor :: Unsupported Lazy Flag");
		}
	...
	function getSF() {
		...
		if ( this.regMaster.bitsDirty & this.bitmaskDirtyGet ) {
			...
			case 1:
				this.value = (this.cpu.resLast >> 7);
				break;
			case 2:
				this.value = (this.cpu.resLast >> 15);
				break;
			case 4:
				this.value = (this.cpu.resLast >> 31);
				break;
	...
	function getWithLookup() {
		...
		if ( this.regMaster.bitsDirty & this.bitmaskDirtyGet ) {
			if ( this.hsh_get[ cpu.insnLast.name ] ) {
				...
				//debugger;
				jemul8.warning("Cannot calculate value for lazy-flag " + this.name
	...
	hsh_getCF[ "SBB" ] = function ( cpu ) {
		jemul8.assert(cpu.insnLast.operand1.size < 4
	...
	= hsh_getCF[ "DEC" ] = function ( cpu ) {
		jemul8.panic("INC & DEC should preserve state!");
		return 0;
	};
