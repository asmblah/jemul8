...
.module("cpu/execute", function ( $ ) { "use strict";
	...
	jemul8.x86CPU.prototype.fetchDecodeExecute = function () {
		...
		// Total no. of instructions executed during this time-slice
			...
			, ticksEndSlice = ticksNow - (ticksNow % ticksSlice) + ticksSlice
		...
		do {
			 ...
			 */
			if ( 1&&(insn = this.cache_insn[ offset ]) ) {
			...
			// Instruction needs to be decoded into cache
				...
				get_prefixes: while ( true ) {
					...
					case 0x0F:
						byt = 0x100 | CS.readSegment(offset++, 1);
						break get_prefixes;
					
					// Segment overrides
					case 0x26: reg_segment = this.ES; break;
					case 0x2E: reg_segment = CS; break;
				...
				if ( !(dataOpcode = this.arr_mapOpcodes[ byt ]) ) {
					dataOpcode = this.arr_mapOpcodeExtensions[ (byt << 3) | nnn ];
				}
				
				// Create new Instruction and store in cache, indexed by address
				//	for fast lookups later
				insn = this.cache_insn[ offsetStart ]
				...
				if ( dataOpcode.length > 1 ) {
					...
					insn.operand1 = new Operand(
						...
						, reg_segment
						, mod, nnn, rm//, this.opcodeHasModRM32[ byt ]
					);
					...
					// TODO: Verify this is right, if so merge in above
					//if ( insn.operand1 ) {
				...
				// Instruction has no operands: segment may be needed though
				// - TODO: Is this needed? Only used by XLAT...?
				} else {
					if ( !insn.reg_segment ) { insn.reg_segment = this.DS; }
				}
			...
			if ( insn.execute ) { insn.execute(this); }
		...
		} while ( /*(insns % 200) || */Date.now() <= ticksEndSlice && !this.isHalted );
