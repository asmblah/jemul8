...
.module("iodev/vga", function ( $ ) { "use strict";
	...
	function VGA( machine ) {
		...
		this.state = {
			misc_output: {
				...
				address: 0x00
				, reg: jemul8.createByteBuffer(0x19)
				, write_protect: false
			}, attribute_ctrl: {
				flip_flop: 0	// 0 = address, 1 = data-write
				, address: 0x00	// Register number
				, video_enabled: false
				, palette_reg: jemul8.createByteBuffer(16)
				, overscan_color: 0x00
				...
				, bitmask: 0x00
				, latch: jemul8.createByteBuffer(4)
			}, sequencer: {
				index: 0x00
				, map_mask: 0x00
				, reset1: false
	...
	VGA.prototype.init = function () {
		...
		this.machine.mem.loadROM(
			//jemul8.getFile("docs/bochs-20100605/bios/VGABIOS-lgpl-latest")
			jemul8.getFile("docs/vgabios-0.6c/VGABIOS-lgpl-latest.debug.bin")
			, 0xC0000, 1
		);
		...
		this.bufVRAM = jemul8.createByteBuffer(state.size_mem);
	...
	VGA.prototype.reset = function ( type ) {
		...
		jemul8.zeroBuffer(this.bufVRAM);
	};
	...
	VGA.prototype.update = function () {
		...
		// Text mode
			...
			if ( state.vga_mem_updated ) {
				...
				var b4 = Date.now();
				this.ctx_screenVGA.clearRect(0, 0, 720, 400);
				
				this.ctx_screenVGA.fillStyle = "#FFFFFF";
				this.ctx_screenVGA.font = MSL + "px monospace";
				
				
				for ( row = 0 ; row < rows ; ++row ) {
					out = "";
					for ( col = 0 ; col < cols ; ++col ) {
						// Display char 0x00 as a space (0x20), otherwise
						//	it will simply be skipped over by .fillText()
						chr = String.fromCharCode(bufVRAM[ addr ] || 0x20);
						//if ( /^[\x00-\x1f]$/.test(chr) ) { chr = "."; }
						out += chr;
						addr += 2;
					}
					this.ctx_screenVGA.fillText(out, 0, y);
					y += MSL;
				}
				...
				/*for ( row = 0 ; row < rows ; ++row ) {
					...
					for ( col = 0 ; col < cols ; ++col ) {
						// Display char 0x00 as a space (0x20), otherwise
						//	it will simply be skipped over by .fillText()
						this.ctx_screenVGA.fillText(
							String.fromCharCode(bufVRAM[ addr ] || 0x20)
							, x, y);
				...
				//var i, s = "";
				//for ( i = 0 ; i < 100 ; i += 2 ) {
				//	s += String.fromCharCode(this.bufVRAM[ start_address + i ]);
				//}
				//debugger;
			}
...
#error Fix the code for plugins
  ...
  if (bx_options.videomode == BX_VIDEO_DIRECT)
  {
	...
	function memoryWrite( device, addrA20, val ) {
		...
		if ( trace ) {
			jemul8.info("VGA memoryWrite() :: 8-bit write to "
		...
		if ( state.graphics_ctrl.graphics_alpha ) {
			$.panic("VGA memoryWrite() :: Gfx mode not implemented yet");
		}
		...
		case 0: /* write mode 0 */
			...
			case 3: // XOR
				...
				break;
			default:
				jemul8.panic("VGA memoryWrite() :: Write mode 0: op = ",
		...
		case 3: // Write mode 3
			...
			break;
		default:
			jemul8.panic("VGA memoryWrite() :: Write mode %u ?",
		...
		if (state.sequencer.map_mask & 0x0f) {
			...
			if (state.sequencer.map_mask & 0x04) {
				if ((offset & 0xe000) == state.charmap_address) {
					...
					if ( trace ) { jemul8.warning("set_text_charbyte()"); }
				}
