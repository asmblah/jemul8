...
.module("iodev/pit/82c54", function ( $ ) { "use strict";
	...
	function PIT_82C54( wrapper ) {
		jemul8.assert(this && (this instanceof PIT_82C54), "PIT_82C54 ctor ::"
	...
	$.extend(PIT_82C54.prototype, {
		print_counter: function ( thisctr ) {
			jemul8.info(("Printing Counter"));
			...
			jemul8.info(("End Counter Printout"));
		}, print_cnum: function ( cnum ) {
			if ( cnum > 0xFF ) {
				jemul8.problem("PIT_82C54.print_cnum() :: Must be 8-bit");
			}
			if ( cnum > MAX_COUNTER ) {
				jemul8.problem(("Bad counter index to print_cnum"));
			} else {
			...
			if ( thisctr.count_LSB_latched || thisctr.count_MSB_latched ) {
				...
				case MSByte_multiple:
					if ( !(this.seen_problems & UNL_2P_READ) ) {
						...
						jemul8.problem(("Unknown behavior when latching during 2-part read."));
						jemul8.problem(("  This message will not be repeated."));
					}
					// [Bochs] I guess latching and resetting to LSB first makes sense;
					jemul8.debug(("Setting read_state to LSB_mult"));
					...
					break;
				default:
					jemul8.problem(("Unknown read mode found during latch command."));
				}
			}
			...
			if ( cnum > MAX_COUNTER ) {
				jemul8.problem(("Counter number too high in clock"));
			} else {
			...
			if ( cnum > MAX_COUNTER ) {
					...
					if ( thisctr.count_written ) {
						if ( thisctr.triggerGATE ) {
							...
							if ( thisctr.write_state == MSByte_multiple ) {
								jemul8.problem(("Undefined behavior when loading a half loaded count."));
							}
						} else {
							this.decrement(thisctr);
							if ( !thisctr.OUTpin ) {
					...
					if ( thisctr.count_written ) {
						if ( thisctr.triggerGATE || thisctr.first_pass ) {
							...
							if ( thisctr.inlatch == 1 ) {
								jemul8.problem(("ERROR: count of 1 is invalid in pit mode 2."));
							}
							if ( !thisctr.OUTpin ) {
								this.set_OUT(thisctr, 1);
							}
				...
				case 3:
					if ( thisctr.count_written ) {
						if ( (thisctr.triggerGATE || thisctr.first_pass
							...
							thisctr.null_count = 0;
							if ( thisctr.inlatch == 1 ) {
								jemul8.problem(("Count of 1 is invalid in pit mode 3."));
							}
							if ( !thisctr.OUTpin ) {
								this.set_OUT(thisctr, 1);
				...
				case 5:
					...
					break;
				default:
					jemul8.problem(("Mode not implemented."));
					thisctr.next_change_time = 0;
					thisctr.triggerGATE = 0;
					break;
				}
			...
			if ( address > MAX_ADDRESS ) {
				jemul8.problem(("Counter address incorrect in data read."));
			} else if ( address == CONTROL_ADDRESS ) {
				jemul8.debug(("PIT Read: Control Word Register."));
				// Read from control word register;
				/* This might be okay.  If so, 0 seems the most logical
				*  return value from looking at the docs.
				*/
				jemul8.problem(("Read from control word register not defined."));
				return 0;
			} else {
				//Read from a counter;
				jemul8.debug(jemul8.sprintf(
				...
				if ( thisctr.status_latched ) {
					...
					if ( thisctr.count_MSB_latched
						...
						jemul8.problem(("Undefined output when status latched and count half read."));
					} else {
					...
					if ( thisctr.count_LSB_latched ) {
						...
						if ( thisctr.read_state == LSByte_multiple ) {
							jemul8.debug(("Setting read_state to MSB_mult"));
							thisctr.read_state = MSByte_multiple;
						}
			...
			if ( address > MAX_ADDRESS ) {
				jemul8.problem(("Counter address incorrect in data write."));
			} else if ( address == CONTROL_ADDRESS ) {
				var SC, RW, M, BCD;
				this.controlword = data;
				jemul8.debug(("Control Word Write."));
				...
				if ( SC == 3 ) {
					...
					var i;
					jemul8.debug(("READ_BACK command."));
					...
					if ( !RW ) {
						...
						switch ( RW ) {
						case 0x1:
							jemul8.debug(("Setting read_state to LSB"));
							thisctr.read_state = LSByte;
							thisctr.write_state = LSByte;
							break;
						case 0x2:
							jemul8.debug(("Setting read_state to MSB"));
						...
						case 0x3:
							...
							break;
						default:
							jemul8.problem(("RW field invalid in control word write."));
						}
				...
				case MSByte:
					...
					break;
				default:
					jemul8.problem(("write counter in invalid write state."));
				}
			...
			if ( cnum > MAX_COUNTER ) {
				jemul8.problem(("Counter number incorrect in 82C54 set_GATE"));
			} else {
				var thisctr = this.counter[ cnum ];
			...
			if ( cnum > MAX_COUNTER ) {
				jemul8.problem(("Counter number incorrect in 82C54 read_OUT"));
				return 0;
			}
			...
			if ( cnum > MAX_COUNTER ) {
				jemul8.problem(("Counter number incorrect in 82C54 read_GATE"));
				return 0;
			}
	...
	function Counter() {
		jemul8.assert(this && (this instanceof Counter), "Counter ctor ::"
			+ " error - constructor not called properly");
