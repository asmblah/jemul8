...
.module("iodev/fdc", function ( $ ) { "use strict";
	...
	function FDC( machine ) {
		jemul8.assert(this && (this instanceof FDC), "FDC ctor ::"
		...
		jemul8.info("FDC (Intel 82077A/82078) PreInit");
		...
		this.state = {
			...
			, command: jemul8.createByteBuffer(10)
			, command_index: 0x00
			, command_size: 0x00
			...
			, result: jemul8.createByteBuffer(10)
			, result_index: 0x00
			, result_size: 0x00
			...
			, cylinder: jemul8.createByteBuffer(4)
			, head: jemul8.createByteBuffer(4)
			, sector: jemul8.createByteBuffer(4)
			, eot: jemul8.createByteBuffer(4)
	...
	$.inherit(FDC, jemul8.IODevice, "FDC"); // Inheritance
	$.extend(FDC, {
		FDD_NONE:         FDD_NONE
		, FDD_525DD:      FDD_525DD
		, FDD_525HD:      FDD_525HD
	...
	FDC.prototype.init = function () {
		...
		for ( idx = 0 ; idx < 4 ; ++idx ) {
			media[ idx ] = new jemul8.FloppyDisk(
		...
		function setupFloppy( idx, devtype, type, status, path ) {
			...
			if ( type !== FLOPPY_NONE && status ) {
				if ( this.evaluate_media(
					...
					disk = media[ idx ];
					jemul8.info(jemul8.sprintf("FDC.init > setupFloppy() ::"
		...
		var drive1 = 1, drive2 = 2, drive3 = 3, floppySignatureCheck = true;
	...
	FDC.prototype.reset = function ( type ) {
		...
		if ( type == jemul8.PC.RESET_HARDWARE ) {
			state.DOR = 0x0c;
			// motor off, drive 3..0
			// DMA/INT enabled
			// normal operation
			...
			jemul8.info("FDC (Floppy Disk Controller) reset in software");
		}
		if ( state.lock == false ) {
			state.config = 0;
			state.pretrk = 0;
	...
	function readHandler( device, addr, io_len ) {
		var state = device.state // "device" will be Floppy
			...
			, drive;
		//debugger;
		//jemul8.debug("Floppy readHandler() :: Read addr = " + $.format("hex", addr));
		...
		case 0x3F5: /* diskette controller data */
			if ( (state.main_status_reg & FD_MS_NDMA)
				...
				jemul8.problem(("port 0x3f5: no results to read"));
		...
		case 0x3F6: // Reserved for future floppy controllers
			...
			jemul8.warning("FDC :: TODO - DEV_hd_read_handler");
			break;
		...
		default:
			jemul8.problem(jemul8.sprintf(
				"FDC (Floppy) readHandler() :: Unsupported read, address=0x%04X!"
				, addr
			));
			return 0;
		}
		//jemul8.debug(jemul8.sprintf(
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		case 0x3F2: /* diskette controller digital output register */
			...
			if ( !dma_and_interrupt_enable ) {
				jemul8.debug(("DMA and interrupt capabilities disabled"));
			}
			...
			jemul8.debug(("io_write: digital output register"));
			jemul8.debug(jemul8.sprintf("  motor on, drive1 = %d", motor_on_drive1 > 0));
			jemul8.debug(jemul8.sprintf("  motor on, drive0 = %d", motor_on_drive0 > 0));
			jemul8.debug(jemul8.sprintf("  dma_and_interrupt_enable=%02x",
				dma_and_interrupt_enable));
			jemul8.debug(jemul8.sprintf("  normal_operation=%02x",
				normal_operation));
			jemul8.debug(jemul8.sprintf("  drive_select=%02x",
				drive_select));
			if ( state.device_type[ drive_select ] == FDRIVE_NONE ) {
				jemul8.debug(("WARNING: non existing drive selected"));
			}
			break;
		...
		case 0x3f4: /* diskette controller data rate select register */
			...
			if ( (val & 0x7c) > 0 ) {
				jemul8.problem(("write to data rate select register: unsupported bits set"));
			}
			break;
		...
		case 0x3F5: /* diskette controller data */
			...
			if ( (state.main_status_reg & FD_MS_NDMA)
				...
				case 0x18: // National Semiconductor version command; return 80h
					...
					jemul8.debug(("io_write: 0x3f5: unsupported floppy command 0x%02x",
						val));
				...
				default:
					jemul8.problem(("io_write: 0x3f5: invalid floppy command 0x%02x"
						, val));
					state.command_size = 0;   // make sure we don't try to process this command
					state.status_reg0 = 0x80; // status: invalid command
					device.enter_result_phase();
			...
			jemul8.debug(("io_write: diskette controller data"));
			return;
		
		case 0x3F6: /* diskette controller (reserved) */
			jemul8.debug("FDC (Floppy) writeHandler() ::"
				+ " Reserved register 0x3f6 unsupported");
			// this address shared with the hard drive controller
			//DEV_hd_write_handler(bx_devices.pluginHardDrive, address, val, io_len);
			jemul8.warning("FDC :: TODO - DEV_hd_write_handler");
			break;
		
		case 0x3F7: /* diskette controller configuration control register */
			if ( (value & 0x03) != state.data_rate ) {
				jemul8.info(("io_write: config control register: 0x%02x", value));
			}
			state.data_rate = value & 0x03;
			switch ( state.data_rate ) {
			case 0: jemul8.debug(("  500 Kbps")); break;
			case 1: jemul8.debug(("  300 Kbps")); break;
			case 2: jemul8.debug(("  250 Kbps")); break;
			case 3: jemul8.debug(("  1 Mbps")); break;
			}
			break;
	...
	/* void */FDC.prototype.floppy_command = function () {
		...
		var str = "";
		for ( i = 0 ; i < state.command_size ; ++i ) {
			str += jemul8.sprintf("[%02x] ", state.command[ i ]);
		}
		jemul8.debug("COMMAND: " + str);
		...
		case 0x03: // specify
			...
			if ( state.main_status_reg & FD_MS_NDMA ) {
				jemul8.problem(("non-DMA mode not fully implemented yet"));
			}
			this.enter_idle_phase();
			return;
		...
		case 0x08: /* sense interrupt status */
			...
			*/
			...
			jemul8.debug(("sense interrupt status"));
			this.enter_result_phase();
			return;
		...
		case 0x4a: // read ID
			...
			if ( motor_on == 0 ) {
				jemul8.problem(("floppy_command(): read ID: motor not on"));
				state.main_status_reg &= FD_MS_NDMA;
			...
			if ( state.media_present[ drive ] == 0 ) {
				jemul8.info(("attempt to read sector ID with media not present"));
				state.main_status_reg &= FD_MS_NDMA;
				state.main_status_reg |= FD_MS_BUSY;
				return; // Hang controller
			}
		...
		case 0x4d: // format track
			...
			motor_on = (state.DOR >> (drive + 4)) & 0x01;
			if ( motor_on == 0 ) {
				jemul8.panic(("floppy_command(): format track: motor not on"));
			}
			state.head[ drive ] = (state.command[ 1 ] >> 2) & 0x01;
			...
			if ( state.media_present[ drive ] == 0 ) {
				jemul8.info(("attempt to format track with media not present"));
				return; // Hang controller
			}
			if ( state.media[ drive ].write_protected ) {
				// media write-protected, return error
				jemul8.info(("attempt to format track with media write-protected"));
			...
			if ( state.main_status_reg & FD_MS_NDMA ) {
				jemul8.debug(("non-DMA floppy format unimplemented"));
			} else {
		...
		case 0xc5: // write normal data, MT=1
			...
			if ( (state.DOR & 0x08) == 0 ) {
				jemul8.panic(("read/write command with DMA and int disabled"));
			}
			drive = state.command[ 1 ] & 0x03;
			state.DOR &= 0xfc;
			state.DOR |= drive;
			...
			motor_on = (state.DOR >> (drive + 4)) & 0x01;
			if ( motor_on == 0 ) {
				jemul8.panic(("floppy_command(): read/write: motor not on"));
			}
			...
			jemul8.debug(("read/write normal data"));
			jemul8.debug(("BEFORE"));
			jemul8.debug(jemul8.sprintf("  drive    = %u", drive));
			jemul8.debug(jemul8.sprintf("  head     = %u", head));
			jemul8.debug(jemul8.sprintf("  cylinder = %u", cylinder));
			jemul8.debug(jemul8.sprintf("  sector   = %u", sector));
			jemul8.debug(jemul8.sprintf("  eot      = %u", eot));
			...
			if ( head != ((state.command[1]>>2)&1) ) {
				jemul8.problem(("head number in command[1] doesn't match head field"));
				state.status_reg0 = 0x40 | (state.head[ drive ] << 2) | drive; // abnormal termination
			...
			if ( cylinder != state.cylinder[ drive ] ) {
				jemul8.debug(("io: cylinder request != current cylinder"));
			...
			if ( logical_sector >= state.media[ drive ].sectors ) {
				jemul8.panic(("io: logical sector out of bounds"));
			}
			...
			if ( (state.command[ 0 ] & 0x4f) == 0x46 ) { // read
				...
				jemul8.panic(("floppy_command(): unknown read/write command"));
				return;
			}
			break;
	...
	/* void */FDC.prototype.floppy_xfer = function (
		...
		if ( state.device_type[ drive ] == FDRIVE_NONE ) {
			jemul8.panic(jemul8.sprintf("floppy_xfer: bad drive #%d", drive));
		}
		...
		if ( direction == FROM_FLOPPY ) {//debugger;
			...
			if ( ret < bytes ) {
				...
				if ( ret > 0 ) {
					...
					jemul8.zeroBuffer(buffer, ret, bytes - ret);
				} else {
					jemul8.info(("read() on floppy image returns 0"));
					// Zero buffer up to "bytes"
					jemul8.zeroBuffer(buffer, 0, bytes);
				}
			}
		// TO_FLOPPY
		} else {
			jemul8.assert(!state.media[ drive ].write_protected);
			...
			if ( ret < bytes ) {
				jemul8.panic(("could not perform write() on floppy image file"));
			}
		}
	};
	...
	/* void */FDC.prototype.timer = function () {
		...
		case 0xc5:
			if ( state.TC ) { // Terminal Count line, done
				...
				jemul8.debug(("<<WRITE DONE>>"));
				jemul8.debug(("AFTER"));
				jemul8.debug(jemul8.sprintf("  drive    = %u", drive));
				jemul8.debug(jemul8.sprintf("  head     = %u", state.head[ drive ]));
				jemul8.debug(jemul8.sprintf("  cylinder = %u", state.cylinder[ drive ]));
				jemul8.debug(jemul8.sprintf("  sector   = %u", state.sector[ drive ]));
		...
		case 0xfe: // (contrived) RESET
			this.reset(jemul8.PC.RESET_SOFTWARE);
		...
		if ( state.pending_command == 0x4d ) { // format track in progress
			...
			case 1:
				if ( data_byte != state.head[ drive ] ) {
					jemul8.problem(("head number does not match head field"));
				}
				break;
			case 2:
	...
	/* void */FDC.prototype.increment_sector = function () {
		...
		if ( (state.sector[ drive ] > state.eot[ drive ])
			...
			if ( state.cylinder[ drive ] >= state.media[ drive ].tracks ) {
				...
				jemul8.info(("increment_sector: clamping cylinder to max"));
			}
		}
	};
	...
	/*void */FDC.prototype.enter_result_phase = function () {
		...
		for ( i = 0 ; i < state.result_size ; ++i ) {
			str += jemul8.sprintf("[%02x] ", state.result[ i ]);
		}
		jemul8.debug("RESULT: " + str);
	};
