...
.module("iodev/dma", function ( $ ) { "use strict";
	...
	function DMA( machine ) {
		jemul8.assert(this && (this instanceof DMA), "DMA ctor ::"
		...
		jemul8.info("DMA (Intel 8237) PreInit");
	...
	$.inherit(DMA, jemul8.IODevice, "DMA"); // Inheritance
	$.extend(DMA, {
	...
	DMA.prototype.raiseHLDA = function () {
		var machine = this.machine, mem = machine.mem
			, idx_channel, channel
			, addrPhysical
			, countExpired = false
			, isMaster = false
			, DMA1 = this.DMA1, DMA2 = this.DMA2
			, core = DMA1;
		...
		if ( channel.mode.typeTransfer === 1 ) {
			...
			if ( !isMaster ) {
				...
				val = fn.call(thisObj);
				mem.writePhysical(addrPhysical, 1, val);
			} else {
				...
				val = fn.call(thisObj);
				mem.writePhysical(addrPhysical, 2, val);
			}
		...
		// Verify
			...
			jemul8.panic("DMA.raiseHLDA() :: typeTransfer > 2 is undefined");
			return;
		}
	...
	DMA.prototype.setDRQ = function ( idx_channel, val ) {
		...
		if ( idx_channel > 7 ) {
			jemul8.panic("DMA.setDRQ() :: idx_channel > 7");
			return;
		}
		...
		channel.DRQ.set(val);
		if ( !channel.inUse ) {
			jemul8.panic("DMA.setDRQ() :: Channel " + idx_channel + " not connected to device");
			return;
		}
		// ( See note above )
		idx_channel &= 0x03;
		...
		jemul8.info("DMA.setDRQ() :: Mask[" + idx_channel + "]: " + $.format("hex", channel.mask));
		jemul8.info("DMA.setDRQ() :: flipFlop: " + core.flipFlop);
		jemul8.info("DMA.setDRQ() :: regStatus: " + $.format("hex", core.regStatus.get()));
		jemul8.info("DMA.setDRQ() :: mode.type: " + $.format("hex", channel.mode.type));
		jemul8.info("DMA.setDRQ() :: decrement_addr: " + $.format("hex", channel.mode.decrement_addr));
		jemul8.info("DMA.setDRQ() :: enable/useAutoInit: " + $.format("bool", channel.mode.useAutoInit));
		jemul8.info("DMA.setDRQ() :: typeTransfer: " + $.format("hex", channel.mode.typeTransfer));
		jemul8.info("DMA.setDRQ() :: addrBase: " + $.format("hex", channel.addrBase));
		jemul8.info("DMA.setDRQ() :: addrCurrent: " + $.format("hex", channel.addrCurrent));
		jemul8.info("DMA.setDRQ() :: countBase: " + $.format("hex", channel.countBase));
		jemul8.info("DMA.setDRQ() :: countCurrent: " + $.format("hex", channel.countCurrent));
		jemul8.info("DMA.setDRQ() :: regPage: " + $.format("hex", channel.regPage.get()));
		...
		if ( channel.mode.type !== DMA_MODE_SINGLE
			...
			jemul8.panic("DMA.setDRQ() :: mode.type "
		...
		if ( (baseDMA & (0x7FFF0000 << isMaster))
			...
			jemul8.info("DMA.setDRQ() :: baseDMA = " + $.format("hex", baseDMA));
			jemul8.info("DMA.setDRQ() :: countBase = " + $.format("hex", channel.countBase));
			jemul8.info("DMA.setDRQ() :: roofDMA = " + $.format("hex", roofDMA));
			// ( NB: isMaster will be converted to int )
			jemul8.panic("DMA.setDRQ() :: Request outside "
				+ $.format("hex", 64 << isMaster) + "k boundary");
			return 0;
		}
	...
	DMA.prototype.registerDMA16Channel = function ( idx_channel, thisObj, fnRead, fnWrite, name ) {
		if ( idx_channel < 4 || idx_channel > 7 ) {
			jemul8.panic("DMA.registerDMA16Channel() ::"
				+ " Invalid channel #" + idx_channel);
			return false; // Fail
		}
		if ( this.DMA2.list_channel[ idx_channel & 0x03 ].inUse ) {
			jemul8.panic("DMA.registerDMA16Channel() :: Channel #"
				+ idx_channel + " already in use");
			return false; // Fail
		}
		jemul8.info("DMA.registerDMA16Channel() :: Channel #"
	...
	DMA.prototype.unregisterDMAChannel = function ( idx_channel ) {
		...
		jemul8.info("DMA.unregisterDMAChannel() :: Channel #"
	...
	function readHandler( device, addr, io_len ) {
		...
		jemul8.info("DMA readHandler() :: Read addr = " + $.format("hex", addr));
		...
		case 0xDA: // DMA-2 Temporary register
			jemul8.problem("DMA-" + (isMaster + 1) + " readHandler() :: Read of temporary register always returns 0");
			return 0;
		...
		case 0x008D:
		case 0x008E:
			jemul8.debug("DMA readHandler() :: Read of extra page register "
				+ $.format("hex", addr) + " (unused)");
		...
		case 0xDE: // DMA-2 undocumented: read all mask bits
			...
			return (0xF0 | result8);
		default:
			jemul8.problem("DMA readHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		//jemul8.info("DMA writeHandler() :: Write to address: "
		...
		default:
			jemul8.problem("DMA writeHandler() :: Unsupported write, address=" + $.format("hex", addr) + "!");
		}
	}
	/* ====== /Private ====== */
