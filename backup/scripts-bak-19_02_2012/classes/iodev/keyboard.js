...
.module("iodev/keyboard", function ( $ ) { "use strict";
	...
	function KeyboardCntrlr( machine ) {
		jemul8.assert(this && (this instanceof KeyboardCntrlr)
			, "KeyboardCntrlr ctor ::"
		...
		jemul8.info("KeyboardCntrlr (Intel 8042) PreInit");
	...
	$.inherit(KeyboardCntrlr, jemul8.IODevice, "KeyboardCntrlr"); // Inheritance
	...
	KeyboardCntrlr.prototype.sendToKeyboard = function ( val ) {
		var keyboard = this.keyboard
			, cps;
		jemul8.debug("KeyboardCntrlr.sendToKeyboard() :: Controller passed byte "
			+ $.format("hex", val) + " to keyboard");
		
		if ( keyboard.bufInternal.expecting_typematic ) {
			keyboard.bufInternal.expecting_typematic = false;
			keyboard.bufInternal.delay = (val >> 5) & 0x03;
			switch ( keyboard.bufInternal.delay ) {
			case 0: jemul8.info("KeyboardCntrlr.sendToKeyboard() ::"
				+ " Setting delay to 250 mS (unused)"); break;
			case 1: jemul8.info("KeyboardCntrlr.sendToKeyboard() ::"
				+ " setting delay to 500 mS (unused)"); break;
			case 2: jemul8.info("KeyboardCntrlr.sendToKeyboard() ::"
				+ " setting delay to 750 mS (unused)"); break;
			case 3: jemul8.info("KeyboardCntrlr.sendToKeyboard() ::"
			...
			jemul8.info("KeyboardCntrlr.sendToKeyboard() :: Setting repeat rate"
				+ " to " + cps + " cps (unused)");
		...
		if ( keyboard.bufInternal.expecting_led_write ) {
			...
			jemul8.debug("KeyboardCntrlr.sendToKeyboard() :: LED status set to "
				+ $.format("hex", keyboard.bufInternal.led_status));
		...
		if ( keyboard.state.expecting_scancodes_set ) {
			...
			if ( val !== 0 ) {
				if ( val < 4 ) {
					...
					jemul8.info("KeyboardCntrlr.sendToKeyboard() ::"
						+ " Switched to scancode set"
						+ keyboard.state.current_scancodes_set + 1
					);
					keyboard.enqueueKey(0xFA); // Send (ACK)nowledge
				} else {
					jemul8.problem("KeyboardCntrlr.sendToKeyboard() ::"
		...
		case 0xF0: // Select alternate scan code set
			...
			jemul8.debug("KeyboardCntrlr.sendToKeyboard() ::"
		...
		case 0xFE:
			jemul8.panic("KeyboardCntrlr.sendToKeyboard() ::"
				+ " Got 0xFE (resend) !!!");
			break;
	...
	KeyboardCntrlr.prototype.sendToMouse = function ( val ) {
		var keyboard = this.keyboard, mouse = this.mouse
			, cps;
		jemul8.debug("KeyboardCntrlr.sendToMouse() :: Controller passed byte "
			+ $.format("hex", val) + " to mouse");
		
		/** NB: This is _always_ a PS/2 mouse **/
		
		jemul8.debug(" -> enable = ", mouse.state.enable);
		jemul8.debug(" -> allow_irq12 = " + keyboard.state.allow_irq12);
		jemul8.debug(" -> aux_clock_enabled = " + keyboard.state.aux_clock_enabled);
		...
		if ( keyboard.state.expecting_mouse_parameter ) {
			...
			case 0xF3: // Set Mouse Sample Rate
				...
				jemul8.debug("KeyboardCntrlr.sendToMouse() ::"
				...
				if ( (val === 200) && (!mouse.state.im_request) ) {
					...
					jemul8.info("KeyboardCntrlr.sendToMouse() ::"
			...
			case 0xE8: // Set Mouse Resolution
				...
				case 3:
					...
					break;
				default:
					jemul8.panic("KeyboardCntrlr.sendToMouse() ::"
			...
			default:
				jemul8.panic("KeyboardCntrlr.sendToMouse() ::"
					...
					+ $.format("hex", keyboard.state.last_mouse_command) + ")");
			}
		} else {
			...
			case 0xE6: // Set Mouse Scaling to 1:1
				...
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Scaling set to 1:1");
				break;
			
			case 0xE7: // Set Mouse Scaling to 2:1
				mouse.enqueueCntrlr(0xFA); // (ACK)nowledge
				mouse.state.scaling = 2;
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Scaling set to 2:1");
				break;
			...
			case 0xEA: // Set Stream Mode
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Stream mode on");
			...
			case 0xEC: // Reset Wrap Mode
				...
				if ( mouse.state.mode === MOUSE_MODE_WRAP ) {
					jemul8.debug("KeyboardCntrlr.sendToMouse() :: Wrap mode off");
			...
			case 0xEE: // Set Wrap Mode
				...
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Wrap mode on");
			...
			case 0xF0: // Set Remote Mode (polling mode, i.e. not stream mode.)
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Remote mode on");
			...
			case 0xF2: // Read Device Type
				...
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Read mouse ID");
				break;
			...
			case 0xF6: // Set Defaults
				...
				mouse.state.mode            = MOUSE_MODE_STREAM;
				mouse.enqueueCntrlr(0xFA); // (ACK)nowledge
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Set defaults");
				break;
			...
			case 0xFF: // Reset
				...
				jemul8.debug("KeyboardCntrlr.sendToMouse() :: Mouse reset");
				break;
			...
			case 0xEB: // Read Data (send a packet when in Remote Mode)
				...
				jemul8.problem("KeyboardCntrlr.sendToMouse() :: Warning:"
					+ " Read Data command partially supported.");
				break;
			
			case 0xBB: // OS/2 Warp 3 uses this command
				jemul8.problem("KeyboardCntrlr.sendToMouse() ::"
	...
	KeyboardCntrlr.prototype.periodic = function ( delta_usecs ) {
		...
		if ( keyboard.bufInternal.num_elements &&
			...
			jemul8.debug("KeyboardCntrlr.periodic() ::"
	...
	KeyboardCntrlr.prototype.updateMouse = function ( x, y ) {
		jemul8.warning("KeyboardCntrlr.updateMouse() :: Not yet implemented");
	};
	...
	function Keyboard( cntrlr ) {
		...
		this.state = {
			...
			, expecting_mouse_parameter: new jemul8.Register( "EXP_MOUSE_PARAM", 1 )
			, last_mouse_command: new jemul8.Register( "LAST_MOUSE_CMD", 1 )
			, timer_pending: false
		...
		this.bufInternal = {
			...
			, delay: new jemul8.Register( "DELAY", 1 )
			, repeat_rate: 0
			, led_status: 0
	...
	Keyboard.prototype.enqueueCntrlr = function ( data ) {
		...
		jemul8.debug("KeyboardCntrlr.enqueueCntrlr(" + $.format("hex", data) + ")");
		
		// See if we need to queue this byte from the controller
		if ( state.outb ) {
			if ( state.sizeQueue >= KBD_CONTROLLER_QSIZE ) {
				jemul8.panic("KeyboardCntrlr.enqueueCntrlr(" + $.format("hex", data)
					+ ") :: state.queue full!");
			}
	...
	Keyboard.prototype.enqueueKey = function ( scancode ) {
		...
		if ( !this.state.outb && this.state.kbd_clock_enabled ) {
			this.cntrlr.activateTimer();
			jemul8.debug("Keyboard.enqueueKey(...) :: Activating timer...");
			return;
		}
	};
	...
	function Mouse( cntrlr ) {
		...
		this.state = {
			...
			, get_status_byte: function () {
				...
				default:
					jemul8.panic("Mouse.state.get_resolution_byte() ::"
	...
	Mouse.prototype.enqueueCntrlr = function ( data ) {
		...
		jemul8.debug("Mouse.enqueueCntrlr(" + $.format("hex", data) + ")");
		
		// See if we need to queue this byte from the controller
		//	(even for mouse bytes)
		if ( state.outb ) {
			if ( state.sizeQueue >= KBD_CONTROLLER_QSIZE ) {
				jemul8.panic("Mouse.enqueueCntrlr(" + $.format("hex", data)
					+ ") :: state.queue full!");
			}
			state.queue[ state.sizeQueue++ ] = data;
			state.sourceQueue = 1;
	...
	Mouse.prototype.enqueueData = function ( data ) {
		...
		if ( !this.cntrlr.state.outb && this.cntrlr.state.aux_clock_enabled ) {
			this.cntrlr.activateTimer();
			jemul8.debug("Mouse.enqueueData(...) :: Activating timer...");
			return;
		}
	};
	...
	function readHandler( device, addr, io_len ) {
		...
		//jemul8.info("KeyboardCntrlr readHandler() :: Read addr = "
		//	+ $.format("hex", addr));
		...
		case 0x60: // Output buffer
			...
			if ( state.auxb ) {
				...
				jemul8.debug("KeyboardCntrlr (for aux - mouse) readHandler() ::"
			...
			// Keyboard byte available
				...
				if ( state.sizeQueue ) {
					...
					jemul8.debug("KeyboardCntrlr readHandler() ::"
			...
			jemul8.debug("num_elements = " + keyboard.bufInternal.num_elements);
			jemul8.debug("KeyboardCntrlr readHandler() ::"
				+ " Read from " + $.format("hex", addr)
				+ " with .outb empty");
		...
		default:
			jemul8.problem("KeyboardCntrlr readHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		jemul8.info("KeyboardCntrlr writeHandler() :: 8-bit write to address: "
		...
		case 0x60: // Input buffer
			...
			if ( state.expecting_port60h ) {
				...
				if ( state.inpb ) {
					jemul8.problem("KeyboardCntrlr writeHandler() ::"
				...
				case 0x60: // Write command byte
					...
					jemul8.debug("KeyboardCntrlr writeHandler() :: allow_irq12 set to "
						+ state.allow_irq12);
					
					if ( !scan_convert ) {
						jemul8.info("KeyboardCntrlr writeHandler() ::"
				...
				case 0xCB: // Write keyboard controller mode
					jemul8.debug("KeyboardCntrlr writeHandler() ::"
				...
				case 0xD1: // Write output port
					...
					if ( !(val & 0x01) ) {
						...
						machine.reset(jemul8.PC.RESET_SOFTWARE);
					}
					break;
				case 0xD4: // Write to mouse
					// I don't think this enables the AUX clock
					//mouse.setClockEnabled(true); // enable aux clock line
				...
				case 0xD2:
					...
					break;
				default:
					jemul8.panic("KeyboardCntrlr writeHandler() ::"
		...
		case 0x64: // Control register
			...
			case 0xAD: // Disable keyboard
				...
				jemul8.debug("KeyboardCntrlr writeHandler() :: Keyboard disabled");
				break;
			case 0xAE: // Enable keyboard
				keyboard.setClockEnabled(true);
				jemul8.debug("KeyboardCntrlr writeHandler() :: Keyboard enabled");
				break;
			...
			break;
		default:
			jemul8.problem("KeyboardCntrlr writeHandler() :: Unsupported read, address="
				+ $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	// Exports
	jemul8.KeyboardCntrlr = KeyboardCntrlr;
});
