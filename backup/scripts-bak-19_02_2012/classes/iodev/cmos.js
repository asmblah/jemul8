...
.module("iodev/cmos", function ( $ ) { "use strict";
	...
	function CMOS( machine ) {
		jemul8.assert(this && (this instanceof CMOS), "CMOS ctor ::"
			+ " error - constructor not called properly");
		
		var idx, state, list_reg;
		
		jemul8.info("CMOS (Intel XXXX?) PreInit");
		...
		for ( idx = 0 ; idx < 128 ; ++idx ) {
			list_reg[ idx ] = new jemul8.Register( "R_" + idx, 1 );
		}
	...
	$.inherit(CMOS, jemul8.IODevice, "CMOS"); // Inheritance
	/*$.extend(CMOS, {
		// Register addresses
		REG_SEC:						0x00
		, REG_SEC_ALARM:				0x01
	...
	CMOS.prototype.init = function () {
		...
		this.machine.mem.loadROM(jemul8.getFile(
		...
		if ( state.timerPeriodic === null ) {	// Continuous, not active
			state.timerPeriodic = machine.registerTimer(handlePeriodicTimer, this, 1000000, true, false, "CMOS");
		}
		if ( state.timerOneSecond === null ) {	// Continuous, not active
			state.timerOneSecond = machine.registerTimer(handleOneSecondTimer, this, 1000000, true, false, "CMOS");
		}
		if ( state.timerUIP === null ) {	// One-shot (not continuous), not active
			state.timerUIP = machine.registerTimer(handleUIPTimer, this, 244, false, false, "CMOS");
		}
		/* ==== /Setup timers ==== */
	...
	CMOS.prototype.saveImage = function () {
		...
		jemul8.info("CMOS.saveImage :: Save not implemented yet.");
	};
	...
	CMOS.prototype.getTimeVal = function () {
		...
		// ...
		jemul8.warning("CMOS.getTimeVal() :: Not yet implemented");
	};
	CMOS.prototype.updateTimeVal = function () {
		var state = this.state;
		
		// ...
		//jemul8.warning("CMOS.updateTimeVal() :: Not yet implemented");
	...
	CMOS.prototype.updateClock = function () {
		// ...
		//jemul8.warning("CMOS.updateClock() :: Not yet implemented");
		var state = this.state;
	...
	CMOS.prototype.CRA_Changed = function () {
		var state = this.state, nibble, dcc;
	...
	function readHandler( device, addr, io_len ) {
		var machine = this.machine
			, state = device.state; // "device" will be CMOS
		var result8; // 8-bit result
		
		jemul8.info("CMOS readHandler() :: Read of CMOS register " + $.format("hex", state.addrMemory));
		//debugger;
		switch ( addr ) {
		case 0x0070:
			// NB: This register is write-only on most machines.
			jemul8.debug("CMOS readHandler() :: Read of index port 0x70. Returning 0xFF");
			return 0xFF;
		case 0x0071:
			...
			return result8;
		default:
			jemul8.panic("CMOS readHandler() :: Unsupported read, address=" + $.format("hex", addr) + "!");
			return 0;
		}
	}
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		jemul8.info("CMOS writeHandler() :: Write to address: "
		...
		case 0x0071:	// Write to current register
			...
			case REG_HOUR_ALARM:
				...
				jemul8.debug("CMOS writeHandler() :: Alarm time changed to " + $.format("time", state.regHourAlarm.get(), state.regMinAlarm.get(), state.regSecAlarm.get()));
				break;
			case REG_SEC:
			case REG_MIN:
			case REG_HOUR:
			...
			case REG_STAT_A: // Control Register A
				...
				dcc = (val >> 4) & 0x07;
				if ( (dcc & 0x06) == 0x06 ) {
					jemul8.info(("CRA: divider chain RESET"));
				} else if ( dcc > 0x02 ) {
					jemul8.panic("CRA: divider chain control " + $.format("hex", dcc));
				}
			...
			case REG_STAT_B: // Control Register B
				...
				if ( val & 0x01 ) {
					...
					jemul8.problem("CMOS writeHandler() :: Write status reg B, daylight savings unsupported");
				}
			...
			case REG_STAT_D: // Control Register D
				jemul8.problem("CMOS writeHandler() :: Write to control register "
					+ $.format("hex", state.addrMemory)
					+ " ignored (read-only)");
				break;
			case REG_DIAGNOSTIC_STATUS:
				jemul8.debug("CMOS writeHandler() :: Write register 0x0e: "
					+ $.format("hex", val));
			...
			case REG_SHUTDOWN_STATUS:
				...
				case 0x00: // Proceed with normal POST (soft reset)
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(00): Shutdown action = normal POST");
					break;
				case 0x01: // Shutdown after memory size check
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(01): Request to change shutdown action"
						+ " to shutdown after memory size check");
					break;
				case 0x02: // Shutdown after successful memory test
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(02): Request to change shutdown action"
						+ " to shutdown after successful memory test");
					break;
				case 0x03: // Shutdown after failed memory test
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(03): Request to change shutdown action"
						+ " to shutdown after successful memory test");
					break;
				case 0x04: // Jump to disk bootstrap routine
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(04): Request to change shutdown action"
						+ " to jump to disk bootstrap routine.");
					break;
				case 0x05: // Flush keyboard (issue EOI) and jump via 40h:0067h
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(05): Request to change shutdown action"
						+ " to flush keyboard (issue EOI) and jump via 40h:0067h.");
					break;
				case 0x06:
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(06): Shutdown after memory test !");
					break;
				case 0x07: // Reset (after failed test in virtual mode)
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(07): Request to change shutdown action"
						+ " to reset (after failed test in virtual mode).");
					break;
				case 0x08: // Used by POST during protected-mode RAM test (return to POST) */
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(08): Request to change shutdown action"
				...
				case 0x09: // Return to BIOS extended memory block move
					...
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(09): Request to change shutdown action"
						+ " to return to BIOS extended memory block move.");
					break;
				case 0x0a: // Jump to DWORD pointer at 40:67
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(0a): Request to change shutdown action"
						+ " to jump to DWORD at 40:67");
					break;
				case 0x0b: // IRET to DWORD pointer at 40:67
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(0b): Request to change shutdown action"
						+ " to iret to DWORD at 40:67");
					break;
				case 0x0c: // RETF to DWORD pointer at 40:67
					jemul8.debug("CMOS writeHandler() :: Reg 0Fh(0c): Request to change shutdown action"
						+ " to retf to DWORD at 40:67");
					break;
				default:
					jemul8.problem("CMOS writeHandler() :: Unsupported shutdown status: " + $.format("hex", val) + "!");
				}
				state.regShutdownStatus.set(val);
				break;
			default:
				jemul8.debug("CMOS writeHandler() :: Write reg "
					+ $.format("hex", state.addrMemory)
			...
			break;
		default:
			jemul8.panic("CMOS IO writeHandler() :: Unsupported write, address=" + $.format("hex", addr) + "!");
			return 0;
		}
	}
	/* ====== /Private ====== */
