...
.module("http", function ( $ ) { "use strict";
	...
	if ( jemul8.support.typedArrays ) {
		jemul8.getFile = function ( path, minSize ) {
			path = getURL(path);
			...
			// Make sure result buffer is of minimum size
				...
				var bufIn = xmlhttp.response
					, bufOut = jemul8.createByteBuffer(minSize);
				jemul8.copyBuffer(bufIn, 0, bufOut, 0, bufIn.byteLength);
				return jemul8.getBuffer(bufOut);
			}
		};
