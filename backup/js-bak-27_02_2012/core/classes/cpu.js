...
define([
	...
	function x86CPU( machine, name_class ) {
		util.assert(this && (this instanceof x86CPU)
			, "x86CPU constructor :: error - not called properly"
		);
	...
	x86CPU.prototype.installStdRegisters = function () {
		...
		this.install(new SegRegister( "XS", 2 ));	// (jemul8-internal)
		...
		this.install(new Register( "GDTR", 4 ));
		// Local Descriptor Table Register
		this.install(new Register( "LDTR", 4 ));
		// Task Register
		this.install(new Register( "TR", 4 ));
		// TODO: This should be a subclass of Register
		this.GDTR.base = this.LDTR.base = 0;
		this.LDTR.limit16 = this.LDTR.limit16 = 0;
	...
	x86CPU.prototype.init = function () {
		//this.decodePage();
		//return;
		...
		function yieldManager() {
			...
			/* ====== Quickly process asynchronous events during Yield ====== */
	...
	x86CPU.prototype.decodePage = function () {
		var i, insn
			, CS = this.CS
	...
	x86CPU.prototype.fetchDecodeExecute = function () {
		...
		// Operand accessor lookups
			, readNonPointer = OperandAccessor.readNonPointer
			, readWithPointer = OperandAccessor.readWithPointer
			, writeNonPointer = OperandAccessor.writeNonPointer
			, writeWithPointer = OperandAccessor.writeWithPointer
		...
		do {
			...
			// Instruction needs to be decoded into cache
				...
				if ( insn.operand1 ) {
					if ( insn.operand1.isPointer ) {
						insn.operand1.read = readWithPointer;//[ insn.operand1.type ];
						insn.operand1.write = writeWithPointer;//[ insn.operand1.type ];
					} else {
						insn.operand1.read = readNonPointer;//[ insn.operand1.type ];
						insn.operand1.write = writeNonPointer;//[ insn.operand1.type ];
					}
					if ( insn.operand2 ) {
						if ( insn.operand2.isPointer ) {
							insn.operand2.read = readWithPointer;//[ insn.operand2.type ];
							insn.operand2.write = writeWithPointer;//[ insn.operand2.type ];
						} else {
							insn.operand2.read = readNonPointer;//[ insn.operand2.type ];
							insn.operand2.write = writeNonPointer;//[ insn.operand2.type ];
						}
						if ( insn.operand3 ) {
							if ( insn.operand3.isPointer ) {
								insn.operand3.read = readWithPointer;//[ insn.operand3.type ];
								insn.operand3.write = writeWithPointer;//[ insn.operand3.type ];
							} else {
								insn.operand3.read = readNonPointer;//[ insn.operand3.type ];
								insn.operand3.write = writeNonPointer;//[ insn.operand3.type ];
							}
						}
					}
				}
			...
			//if ( asm === "ADD b,DS:[BX+SI], AL" ) { debugger; }
			...
			//if ( this.IP.get() === 0x79A9 ) { debugger; } // []
			...
			//var PC = ((CS.get() << 4) + EIP.get());
			//if ( PC > 0xFFFF0 ) {
				//debugger;
			//}
			
			++insns;
		// Stop CPU loop for this slice if we run out of time or CPU halts
		} while ( ((insns % 20) || (Date.now() <= ticksEndSlice)) && !this.isHalted );
		//} while ( insns < 25000 && !this.isHalted );
	...
	x86CPU.prototype.handleAsynchronousEvents = function () {
		var idx, list, len
		...
		// Check interrupts are enabled/uninhibited & one is pending
			...
			// TODO: Should we have a loop here to process > 1 interrupt?
	...
	// See end of .fetchDecodeExecute() / CPU loop
	x86CPU.prototype.handleSynchronousEvents = function () {
		// Resume Flag
		if ( this.RF.get() ) {
			this.RF.clear();
		} else {
	...
	x86CPU.prototype.pushStack = function ( val, len ) {//debugger;
		...
		if ( len === 1 ) {
			...
			if ( val >> 23 ) { val |= 0xFF000000; }
			len = 4;
		}
		
		mask = util.generateMask(len);
		
		// Decrement by operand size
		ptrStack = (ptrStack - len) & mask;
	...
	x86CPU.prototype.popStack = function ( len ) {
		// Pointer to top of Stack
		var SP = this.SS.cache.default32BitSize ? this.ESP : this.SP
			, ptrStack = SP.get()
			, mask, res;
	...
	x86CPU.prototype.interrupt = function ( num ) {
		...
		if ( offsetVector > sizeIDT ) {
			...
			util.problem("CPU.interrupt() :: Error - interrupt vector"
				+ " is out of IDT range");
		}
		
		// FIXME: Is INT 0x00 called for divide-by-zero or not?
		// - no, need an Exceptions system (see Bochs' /cpu/exception.cc)
		if ( num === 0x00 ) {
	...
	x86CPU.prototype.interruptReturn = function ( is32 ) {
		var eflags;
		if ( !is32 ) {
