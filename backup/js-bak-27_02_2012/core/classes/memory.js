...
define([
	...
	, "./memory/buffer"
	, "./memory/rom"
], function ( util, SegRegister, IODevice, Buffer, ROM ) { "use strict";
	
	var poly_buf_readSegment, poly_buf_writeSegment
	...
	Memory.prototype.readPhysical = function ( addrPhysical, len ) {
		// FIXME: Segment system expects access to be within its page,
		//        need to extract memory map from SegRegister.set()
		//        below (this would only be valid for physical addresses
		//        within 1st 64KB page
	...
	SegRegister.prototype.set = function ( segment ) {
		...
		// Protected mode
		if ( cpu.PE.get() ) {
			util.panic("No protected mode support yet.");
		// Real mode
		} else {
			/** NB: DON'T reset cache fields to default
			    - this will allow for Unreal mode **/
		}
		...
		// ROMs & lower/upper BIOS areas
			...
			// CMOS loaded in segment 0xF000 (real mode)
	...
	= function ( offset, len ) {
		util.problem("SegRegister.readSegment() :: Read not allowed!");
	};
	/* poly */SegRegister.prototype.writeSegment
	= function ( offset, val, len ) {
		util.problem("SegRegister.writeSegment() :: Write not allowed!");
		debugger;
	};
	
	function poly_handler_readSegment( offset, len ) {
		// (See above)
		// util.assert(len <= 4, "MemoryAccessor.readSegment() :: Memory system"
		//	+ " only supports <= 32-bit addresses");
		// util.assert(offset >= 0 && offset <= 0xFFFF
	...
	function poly_handler_writeSegment( offset, val, len ) {
		...
		// util.assert(len <= 4, "MemoryAccessor.writeSegment() :: Memory system"
		//	+ " only supports <= 32-bit addresses");
		// util.assert(offset >= 0 && offset <= 0xFFFF
	...
	if ( 1||util.support.typedDataView ) {
	...
	//	is not available)
		...
		poly_buf_readSegment = function ( offset, len ) {
			...
			case 4:	// Dword ( 32-bit )
				return (buf[ addrA20 + 3 ] << 24)
					...
					| (buf[ addrA20 ]);
			default:
				util.problem("poly_buf_readSegment() ::"
					+ " Operand-size > 32-bit");
			}
		};
		// Write to memory buffer (little-endian)
		poly_buf_writeSegment = function ( offset, val, len ) {
			...
			case 4:	// Dword (32-bit / 4-bytes)
				...
				return;
			default:
				util.problem("poly_buf_writeSegment() ::"
					+ " Operand-size > 32-bit");
			}
		};
	}
