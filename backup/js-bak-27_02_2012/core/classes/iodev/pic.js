...
define([
	...
	PIC.prototype.lowerIRQ = function ( irq ) {
		...
		if ( (irq <= 7) && (state.PICmaster.inIRQ & mask) ) {
			util.debug("PIC.lowerIRQ() :: IRQ line #"
	...
	PIC.prototype.raiseIRQ = function ( irq ) {
		...
		// Master PIC handles IRQs <= 7
		if ( (irq <= 7) && !(state.PICmaster.inIRQ & mask) ) {
			util.debug("PIC.raiseIRQ() :: IRQ line #"
	...
	PIC.prototype.setMode = function ( forMaster, mode ) {
		...
		util.debug("PIC(" + core.name + ").setMode() ::"
	...
	PIC.prototype.acknowledgeInterrupt = function () {
		...
		util.debug("PIC.acknowledgeInterrupt() :: IRQ #" + irq + " acknowledged");
		return vector;
	};
	...
	PIC_Core.prototype.initCommand = function ( val ) {
		...
		if ( val & 0x10 ) {
			util.debug("PIC(" + this.name + ") writeHandler() ::"
				+ " Init command 1 found");
			util.debug(" -> requires 4 = " + (val & 0x01));
			util.debug(" -> cascade mode: [0=cascade, 1=single] "
				+ ((val & 0x02) >> 1));
			this.init.inInit = 1;
			...
			this.doRotateOnAutoEOI = false;
			if ( val & 0x02 ) { return util.panic("PIC(" + this.name + ")"
		...
		if ( core.init.inInit ) {
			...
			case 4:
				...
				if ( val & 0x02 ) {
					...
					util.debug(" -> Normal EOI interrupt");
					core.useAutoEOI = false;
				}
				if ( val & 0x01 ) {
					util.debug(" -> 80x86 mode");
				} else {
	...
	function readHandler( device, addr, io_len ) {
		...
		case 0x0020: // Read from Master "read register"
			...
			util.debug("PIC(master) readHandler() :: Read Master "
		...
		case 0x0021: // Read from Master IMR "Interrupt Mask Register"
			...
			util.debug("PIC(master) readHandler() :: Read Master IMR = "
				+ util.format("hex", result8));
			return result8;
		case 0x00A0: // Read from Slave "read register"
			result8 = state.PICslave.regReadSelect.get();
			util.debug("PIC(slave) readHandler() :: Read Master "
		...
		case 0x00A1: // Read from Slave IMR "Interrupt Mask Register"
			...
			util.debug("PIC(slave) readHandler() :: Read Master IMR = "
				+ util.format("hex", result8));
			return result8;
		default:
