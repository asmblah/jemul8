...
define([
	...
	function DMA( machine ) {
		...
		util.debug("DMA constructor() :: Channel 4 in use by cascade");
	}
	...
	DMA.prototype.raiseHLDA = function () {
		...
		//util.debug("DMA.raiseHLDA() :: OK in response to DRQ " + idx_channel);
		// Cache reference
	...
	DMA.prototype.setDRQ = function ( idx_channel, val ) {
		...
		util.info("DMA.setDRQ() :: Mask[" + idx_channel + "]: " + util.format("hex", channel.mask));
		util.info("DMA.setDRQ() :: flipFlop: " + core.flipFlop);
		util.info("DMA.setDRQ() :: regStatus: " + util.format("hex", core.regStatus.get()));
		util.info("DMA.setDRQ() :: mode.type: " + util.format("hex", channel.mode.type));
		util.info("DMA.setDRQ() :: decrement_addr: " + util.format("hex", channel.mode.decrement_addr));
		util.info("DMA.setDRQ() :: enable/useAutoInit: " + util.format("bool", channel.mode.useAutoInit));
		util.info("DMA.setDRQ() :: typeTransfer: " + util.format("hex", channel.mode.typeTransfer));
		util.info("DMA.setDRQ() :: addrBase: " + util.format("hex", channel.addrBase));
		util.info("DMA.setDRQ() :: addrCurrent: " + util.format("hex", channel.addrCurrent));
		util.info("DMA.setDRQ() :: countBase: " + util.format("hex", channel.countBase));
		util.info("DMA.setDRQ() :: countCurrent: " + util.format("hex", channel.countCurrent));
		util.info("DMA.setDRQ() :: regPage: " + util.format("hex", channel.regPage.get()));
	...
	function readHandler( device, addr, io_len ) {
		...
		case 0x008D:
		case 0x008E:
			util.debug("DMA readHandler() :: Read of extra page register "
