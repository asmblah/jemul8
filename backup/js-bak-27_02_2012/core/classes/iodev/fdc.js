...
define([
	...
	function readHandler( device, addr, io_len ) {
		var state = device.state // "device" will be Floppy
			...
			, drive;
		//debugger;
		//util.debug("Floppy readHandler() :: Read addr = " + util.format("hex", addr));
	...
	function writeHandler( device, addr, val, io_len ) {
		...
		case 0x3F2: /* diskette controller digital output register */
			...
			if ( !dma_and_interrupt_enable ) {
				util.debug(("DMA and interrupt capabilities disabled"));
			}
			...
			util.debug(("io_write: digital output register"));
			util.debug(util.sprintf("  motor on, drive1 = %d", motor_on_drive1 > 0));
			util.debug(util.sprintf("  motor on, drive0 = %d", motor_on_drive0 > 0));
			util.debug(util.sprintf("  dma_and_interrupt_enable=%02x",
				dma_and_interrupt_enable));
			util.debug(util.sprintf("  normal_operation=%02x",
				normal_operation));
			util.debug(util.sprintf("  drive_select=%02x",
				drive_select));
			if ( state.device_type[ drive_select ] == FDRIVE_NONE ) {
				util.debug(("WARNING: non existing drive selected"));
			}
			break;
		...
		case 0x3F5: /* diskette controller data */
			...
			if ( (state.main_status_reg & FD_MS_NDMA)
				...
				case 0x18: // National Semiconductor version command; return 80h
					...
					util.debug(("io_write: 0x3f5: unsupported floppy command 0x%02x",
						val));
			...
			util.debug(("io_write: diskette controller data"));
			return;
		
		case 0x3F6: /* diskette controller (reserved) */
			util.debug("FDC (Floppy) writeHandler() ::"
		...
		case 0x3F7: /* diskette controller configuration control register */
			...
			case 0: util.debug(("  500 Kbps")); break;
			case 1: util.debug(("  300 Kbps")); break;
			case 2: util.debug(("  250 Kbps")); break;
			case 3: util.debug(("  1 Mbps")); break;
			}
			break;
	...
	/* void */FDC.prototype.floppy_command = function () {
		...
		case 0x08: /* sense interrupt status */
			...
			*/
			...
			util.debug(("sense interrupt status"));
			this.enter_result_phase();
			return;
		...
		case 0x4d: // format track
			...
			if ( state.main_status_reg & FD_MS_NDMA ) {
				util.debug(("non-DMA floppy format unimplemented"));
			} else {
		...
		case 0xc5: // write normal data, MT=1
			...
			util.debug(("read/write normal data"));
			util.debug(("BEFORE"));
			util.debug(util.sprintf("  drive    = %u", drive));
			util.debug(util.sprintf("  head     = %u", head));
			util.debug(util.sprintf("  cylinder = %u", cylinder));
			util.debug(util.sprintf("  sector   = %u", sector));
			util.debug(util.sprintf("  eot      = %u", eot));
			...
			if ( cylinder != state.cylinder[ drive ] ) {
				util.debug(("io: cylinder request != current cylinder"));
	...
	/* void */FDC.prototype.timer = function () {
		...
		case 0xc5:
			if ( state.TC ) { // Terminal Count line, done
				...
				util.debug(("AFTER"));
				util.debug(util.sprintf("  drive    = %u", drive));
				util.debug(util.sprintf("  head     = %u", state.head[ drive ]));
				util.debug(util.sprintf("  cylinder = %u", state.cylinder[ drive ]));
				util.debug(util.sprintf("  sector   = %u", state.sector[ drive ]));
