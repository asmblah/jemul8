...
define([
	...
	function getWithLookup() {
		...
		if ( this.regMaster.bitsDirty & this.bitmaskDirtyGet ) {
			...
			case 1:
				if ( cpu.valLast1 >> 7 ) {
					cpu.valLast1 |= 0xFFFFFF00;
					cpu.valLast1 >>= 0;
				}
				break;
			case 2:
				if ( cpu.valLast1 >> 15 ) {
					cpu.valLast1 |= 0xFFFF0000;
					cpu.valLast1 >>= 0;
				}
				break;
			default:
	...
	hsh_getOF[ "NEG" ] = hsh_getOF[ "INC" ] = function ( cpu ) {
		...
		switch ( cpu.insnLast.operand1.size ) {
		case 1:
			return (cpu.resLast & 0xFF) === 0x80;
		case 2:
			return (cpu.resLast & 0xFFFF) === 0x8000;
		case 4:
			return (cpu.resLast & 0xFFFFFFFF) === 0x80000000;
		default:
	...
	hsh_getOF[ "DEC" ] = function ( cpu ) {
		...
		switch ( cpu.insnLast.operand1.size ) {
		case 1:
			return (cpu.resLast & 0xFF) === 0x7F;
		case 2:
			return (cpu.resLast & 0xFFFF) === 0x7FFF;
		case 4:
			return (cpu.resLast & 0xFFFFFFFF) === 0x7FFFFFFF;
		default:
			jemul8.panic("Invalid operand size");
		}
	};
