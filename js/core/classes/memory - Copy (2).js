...
define([
    ...
    Memory.prototype.readPhysical = function ( addrPhysical, size ) {
        return this.accessPhysical(addrPhysical, TYPE_READ, size);
    };
    Memory.prototype.writePhysical = function ( addrPhysical, val, size ) {
        this.accessPhysical(addrPhysical, TYPE_WRITE, size, val);
    };
    ...
    // Read from or write to Physical address (simulates the address bus
    //  connection from CPU->MMU/Northbridge)
    Memory.prototype.accessPhysical = function ( addrPhysical, type, size, val ) {
        ...
        // Reads/write via handler function(s)
        } else {
            if ( type === "read" ) {
